...
metaTitle: Using StackBuilder Plus to Perform a Minor Version Update
...
StackBuilder Plus welcome window opens (shown in the following figure).

![*The StackBuilder Plus Welcome
Window*](images/the_stackBuilder_plus_welcome_window.png)
...
access, use the `Proxy Servers` on the `Welcome to StackBuilder Plus
Window!` to open the `Proxy servers` dialog (shown in the following
figure).
...
window*](images/the_stackBuilder_plus_module_selection_window.png)

The tree control on the StackBuilder Plus module selection window (shown
in the following figure) displays a node for each module category. To
perform an Advanced Server update, expand the `Database Server` module
in the tree control and check the box to the left of the Advanced Server
upgraded version. Then, click `Next`.
...
![*The StackBuilder Plus registration
window*](images/the_stackBuilder_plus_registration_window.png)

![*A summary window displays a list of selected
packages*](images/selected_packages_summary_window.png)

StackBuilder Plus confirms the packages selected. The `Selected
packages` dialog will display the name and version of the installer;
...
complete*](images/download_complete_confirmation.png)
...
installation*](images/stackBuilder_plus_confirms_the_completed_installation.png)

When the upgrade is complete, StackBuilder Plus will alert you to the
success or failure of the installation of the requested package. If you
were prompted by an installer to restart your computer, reboot now.
...
If the update fails to install, StackBuilder Plus will alert you to the
