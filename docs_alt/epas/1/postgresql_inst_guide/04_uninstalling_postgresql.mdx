---
title: Uninstalling PostgreSQL
metaTitle: Uninstalling PostgreSQL
metaDescription: description
---

The PostgreSQL installer creates an uninstaller in the PostgreSQL
installation directory.

## Uninstalling PostgreSQL on a Windows System

You can use the graphical interface provided by Windows to uninstall
PostgreSQL. Navigate through the Windows `Control Panel` to open the
Windows `Uninstall or change a program` dialog.

![*The Uninstall or change a program
dialog*](images/the_uninstall_or_change_a_program_dialog.png)

Right click on `PostgreSQL 12`, and select `Uninstall/Change` from the
context menu.

![*Confirm that you wish to uninstall
PostgreSQL*](images/uninstalling_postgreSQL_comfirmation.png)

If you wish to remove the `Entire application`, click `Next` to
continue. If you choose to remove `Individual components`, a selection
screen opens, allowing you to select which components you wish to
uninstall.

![*Select the components to
uninstall*](images/select_the_components_to_uninstall.png)

Select the components you wish to uninstall, and click `Next` to start
uninstalling components.

![*Uninstalling PostgreSQL*](images/uninstalling_postgreSQL.png)

A progress bar will keep you informed as PostgreSQL is removed.

![*An Info dialog confirms the
uninstallation*](images/uninstallation_confirmation.png)

When the uninstaller completes, an `Info` dialog opens to confirm. Click
`OK` to exit.

## Uninstalling PostgreSQL on a Mac System

To uninstall PostgreSQL on a Mac system, assume the identity of an
operating system superuser, and navigate into the folder in which the
uninstaller resides:

> `/Library/PostgreSQL/12`

Then, invoke the uninstaller with the command:

> `open uninstall-postgres.app`

If prompted, provide a password that allows the uninstaller to make
changes to your system. The uninstaller will open, asking you if you
wish to uninstall the `Entire application` or `Individual components`.

![*Remove the entire application or
components*](images/remove_the_entire_application_or_components.png)

If you wish to remove the `Entire application`, click `Next` to
continue. If you choose to remove `Individual components`, a selection
screen opens, allowing you to select which components you wish to
uninstall.

![*Selecting the components to
uninstall*](images/selecting_the_components_to_uninstall.png)

Select the components you wish to uninstall, and click `Next` to start
uninstalling components. Progress bars are displayed as each component
is removed; an `Info` popup informs you when the uninstallation is
complete.

![*The Uninstallation is
completed*](images/uninstallation_completed.png)

Click `OK` to exit the uninstaller.
