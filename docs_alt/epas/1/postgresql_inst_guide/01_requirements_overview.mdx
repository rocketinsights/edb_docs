...
clicking on the name of the installer and selecting `Run as
administrator` from the context menu. If prompted, enter an
...
(`.dmg`) file from the website or as an archive (`.zip`) from Stack
Builder. To extract the installer, simply mount the disk image and copy
the installer to the desired location, or run it directly from the disk
image.
