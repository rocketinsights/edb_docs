...
---
title: The User Tab
metaTitle: The User Tab
metaDescription: description
---

Fields on the `User` tab allow you to view or modify information about
the current user.

![*The User tab of the Ark console.*](images/ark_user_tab.png)

To change the `First Name`, `Last Name`, or `Company Name` of the
registered user, modify the corresponding fields and click the `Apply
Changes` button. A popup will confirm that the changes have been
applied.

The `Notification Email` field on the `User` tab displays the default
email address that will be used for cluster notifications unless an
alternate address is provided. You can optionally:
...
the change of address. Enter your password, and click `Confirm` to
modify the address, or click `Cancel` to exit the popup without applying
the change.
...
Password, Updating on AWS

</div>

## Updating a Password on Amazon AWS

If your Ark console is deployed on Amazon AWS, the `User` tab displays
the Amazon Role ARN associated with your Ark user account, and provides
an option that allows you to modify your password. To modify your
...
Click the `Confirm` button to change the password to the new value;
click `Cancel` to exit the dialog without modifying the password.
