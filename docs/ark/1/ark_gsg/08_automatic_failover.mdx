...
Manually Promoting a Standby promote

</div>

## Manually Promoting a Standby Node

You can use the `Promote` option on a cluster's context menu to manually
promote a standby node to master node. When you promote the standby, the
master node is deleted. You must initiate the promotion from a console
that resides in the same region as the standby that is being promoted.

To promote a node to master, right-click on the cluster name in the
`Details` panel, and select `Promote` from the context menu.

![*The Promote option on a context
menu*](images/manual_promote_context.png)

When you select the `Promote` option, the console will ask you to
confirm that you wish to promote the standby to master and delete the
original master node.

![*Confirming that you wish to promote a
standby*](images/manual_promote_confirmation.png)

Click `Continue` to instruct Ark to perform the promotion. During the
promotion, status messages will inform you of the state of the cluster.

![*Status messages display the state of the
cluster*](images/manual_promote_status.png)
