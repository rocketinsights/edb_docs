...
<!-- end list -->
  ...
  - \- Use the `pg_dump` and `pg_restore` utilities to move the cluster into a  
    ...
    To move to a larger server class, use the pg_dump utility to make a backup of the cluster. After backing up the cluster, create a new instance with the larger server class, and use pg_restore to restore the cluster on the new instance. For information about using pg_dump and pg_restore, see
