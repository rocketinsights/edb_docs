...
| ![icon16](../images/delete_icon.png)           | Use the `Delete Cluster` icon to delete the currently selected cluster. A popup dialog will ask you to confirm you wish to terminate the cluster; once terminated, a cluster may only be restored from a backup. By default, the box next to `Release elastic IP address` is checked. Deselect this option if you wish to retain the IP address for re-use with other clusters. If you release the IP address, it will be made available for use by other clusters. When you terminate an active cluster, backups are not deleted. Backups (including user data) are retained until they are selected and deleted from the Backups tab. |
  ...
  - Select the [Details bar ](../04_using_ark_console/02_ark_clusters_tab#details_panel) to view information about the state of the selected cluster.
  - Select the [Monitoring bar ](../04_using_ark_console/02_ark_clusters_tab#monitoring_panel) to view usage statistics for the selected cluster.
  - Select the [Events bar ](../04_using_ark_console/02_ark_clusters_tab#events_panel) to review event logs describing activities on the selected cluster.
