...
Use the `Auto Scale Options` controls to adjust the threshold at which
EDB Ark automatically scales up cluster resources. The `Auto Scale
Options` controls are located on the `Details` panel; to access the
`Details` panel, navigate to the `Clusters tab <ark_clusters_tab>`, and
highlight the name of a cluster.

![*The Auto Scale Options
controls.*](images/auto_scale_options_dialog.png)
...
values, EDB Ark will display a `New Value Saved` notice, alerting you
