...
with self-registration enabled, see Using Self-Registration on an Amazon
Hosted Console.
...
Enter your user name in the `Email` field, and the associated password
in the `Password` field, and click `Log In` to connect to the Ark
console.
...
Navigate to the `Roles` dashboard, and click the `Create New Role`
button.

![*Provide a role name.*](images/self_reg_set_role_name.png)

When the `Set Role Name` dialog opens, specify a name for the new role
and click `Next Step` to specify a role type.

![*Specify that the role allows EC2 instances to call AWS
services.*](images/self_reg_role_type.png)

On the `Select Role Type` dialog, select the `AWS Service Roles` radio
button, and then the `Select` button to the right of `Amazon EC2` to
continue to the `Attach Policy` dialog.

![*The Attach Policy dialog.*](images/self_reg_attach_policy.png)

When the `Attach Policy` dialog opens, do not specify a policy; instead,
click `Next Step` to continue to the `Review` dialog.

![*Review the role information.*](images/self_reg_review.png)

When the `Review` dialog opens, review the information displayed, and
then click `Create Role` to instruct the AWS management console to
create the described role.

![*The new role is displayed on the Roles
page.*](images/self_reg_create_role.png)

The role will be displayed in the role list on the `Amazon IAM Roles`
page. The `Summary` tab will display a `Role ARN`, but the ARN will not
be enabled until the security policy and trust policy are updated.
...
`Permissions` tab and open the `Inline Policies` menu. Select `Click
here` to add a new policy.

![*The Inline Policies menu.*](images/self_reg_inline_policy.png)

When the `Set Permissions` dialog opens, select the `Custom Policy`
radio button, and then click the `Select` button.

![*Add a Custom Policy.*](images/self_reg_set_permissions.png)

![*Provide the policy name and
contents.*](images/self_reg_custom_policy.png)

Use the fields on the `Set Permissions` dialog to define the security
policy:
  ...
  - Copy the security policy text into the `Policy Document` field. The
    security policy required by Ark is available in `AWS
...
the file available in `AWS Resources <aws_policies>` .
...
to the `Log In` page of your Ark console. Click the `Register` button to
open the `New User Registration` dialog.
...
panel.*](images/self_reg_role_detail.png)
...
dialog into the trust policy, replacing `EDB-PPCD-CONSOLE`. Please note
...
cloud.*](images/self_reg_step_two.png)
...
After completing the registration, you can use the `Login/Register`
dialog to access the Ark console.
...
associated password in the `Password` field, and click `Log In` to
connect to the Ark console.
