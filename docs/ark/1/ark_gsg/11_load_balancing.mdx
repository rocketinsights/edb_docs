...
To enforce load balancing of `SELECT` statements with leading white
space or comments, modify the `pgpool.conf` file, and set the
`ignore_leading_white_space` parameter to `true`.

You can also use the `black_list` and `white_list` parameters (located
in the `pgpool.conf` file) to instruct pgPool to direct specific
statements or functions to the master node. This is useful for cases
where a `SELECT` statement (normally directed to a replica) calls a
function that in turn might modify the database, and so should be
