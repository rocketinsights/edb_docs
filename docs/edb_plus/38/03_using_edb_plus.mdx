...
Login information for connecting to the database server and database. `login` takes the following form; please note that there must be no white space within the login information:
...
The following example demonstrates using variable `hr_5445` in the `login.sql` file, and connecting to database `hr` on `localhost` at port `5445`.
