...
To open Capacity Manager, select the `Capacity Manager...` option from
the PEM client `Management` menu; the Capacity Manager wizard opens,
displaying a tree control on the `Metrics` tab.

![*The Capacity Manager dialog*](images/capacity_manager_dialog.png)

Expand the tree control on the `Metrics` tab to review the metrics for
the node that you wish to analyze. Check the box to the left of the name
of the metric to include the metric in your report.

![*The Metrics tree control*](images/capacity_manager_metrics.png)
...
Click the `Generate` button to display the report onscreen (accepting
the default configuration options), or use the Options tab to customize
sampling boundaries, report type and report destination. Please note
that the times displayed on the Options tab are the time zone in which
the PEM client resides.

![*Specify the time period, type, and destination of the
report*](images/capacity_manager_report.png)
...
report:
  ...
  - Use the date and time selectors next to the `Start time` field to
    ...
    the number of Historical day(s) of data to include in the report.
    The date and time specified in the Start time field must not be
    later than the current date/time.
  ...
  - The end boundary for the report can be a time, a number of days in
    ...
    to the `End time` field to specify an end boundary for the report,
    or select the number of Extrapolated day(s) of data to include in
    the report. The time specified in the End time field must be later
    than the time specified in the Start time field.
...
cm_max_date_in_years parameter. By default,
cm_max_end_date_in_years is 5; you can use the Server Configuration
dialog to modify the value of cm_max_end_date_in_years.
...
produced by Capacity Manager:
  ...
  - Select `Table of data` to instruct Capacity Manager to display a
    table containing the report data in the PEM client window.
...
where to display or save the report:
  ...
  - Select `New` tab to instruct Capacity Manager to display the report
    on a new tab in the PEM client. You must select New tab to display
    the first generation of a Capacity Manager report; for subsequent
...
destination of the Capacity Manager report, click the `Generate` button
...
![*Saving a Capacity Manager
Template*](images/save_capacity_manager_template.png)

3.  Provide a report name in the `Title` field, select a location to
    store the template in the tree control.
4.  Click `OK`.
...
Use the `Manage Templates` button open a dialog that allows you to
rename or remove unwanted templates.
