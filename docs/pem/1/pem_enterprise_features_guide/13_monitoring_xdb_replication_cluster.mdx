...
metaTitle: Monitoring an xDB Replication Cluster
...
By default, the `xDB Replication` probe is disabled. To enable the `xDB
Replication` probe, right click on the name of the server, and select
`Connect` from the context menu; if prompted, provide authentication
information. After connecting, expand the server node of the tree
control, and highlight the name of the replicated database. Then, select
`Manage Probes...` from the `Management` menu.
...
probe:
  ...
  - Use the `Enabled?` slider to instruct PEM to execute the xDB
    Replication probe.
  - Set the `Default` slider in the `Data Retention` field to No to
    modify the number of days that PEM will store the information
