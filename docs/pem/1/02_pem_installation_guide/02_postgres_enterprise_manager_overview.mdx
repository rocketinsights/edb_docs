...
Postgres Enterprise Manager (PEM) consists of components that provide the management and analytical features of PEM:
  ...
  - **SQL Profiler plugin:** This plugin to the Postgres server is used to generate the monitoring data used by the SQL Profiler tool. Installation of the SQL Profiler plugin is optional, but the plugin must be installed into each instance of Postgres you wish to profile. The SQL Profiler may be used with any supported version of an EnterpriseDB distribution of a PostgreSQL server or an Advanced Server (not just those managed through the PEM server). See [Installing the SQL Profiler Plugin ](05_installing_the_sql_profiler_plugin#installing_the_sql_profiler_plugin) for details and supported versions.
