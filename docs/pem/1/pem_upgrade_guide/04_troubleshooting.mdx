...
utility may restore the `pem.alert` pre-requisites *after* it attempts
to restore `pem.alert`.

If this happens, the output from `pg_restore` will include error
messages that refer to the `alert` table:
...
If you encounter this problem, restore the `pem` database before
restoring the `pem.alert` table. Restoring the `pem` database will
install the pre-requisites for `pem.alert`, and the restoration of the
