...
the management and analytical features of PEM:
  ...
  - **PEM Agent:** The PEM agent is responsible for executing tasks and
    ...
    (`pemworker.exe`) and the PEM agent (`pemagent.exe`). Each PEM
    worker has a corresponding PEM agent that you can use to start or
    stop the PEM worker. The PEM agent will also restart the PEM worker
...
<!-- end list -->
  ...
  - \- **SQL Profiler plugin:** This plugin to the Postgres server is
    used to  
...
[Installing the SQL Profiler Plugin](installing_the_sql_profiler_plugin)
...
For information about modifying the `pg_hba.conf` file, see the *PEM
...
that the host contains the `epel-release` packages. To install the
packages, open a command line, assume `root` privileges, and invoke the
commands:
