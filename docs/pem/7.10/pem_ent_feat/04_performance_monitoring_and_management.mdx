...
-   Batch - the probe will use a command-script or shell-script to gather information.
    ...
    Before creating a batch probe on a Linux system, you must modify the agent.cfg file, setting the allow\_batch\_probes parameter equal to true and restart the PEM agent. The `agent.cfg` file is located in `/opt/PEM/agent/etc`.
