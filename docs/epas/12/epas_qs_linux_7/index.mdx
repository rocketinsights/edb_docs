...
> 
> **Supporting Tools, Utilities, and Clients** - EnterpriseDB makes available a full suite of tools and utilities that can help you monitor and manage your Advanced Server deployment. For more information, visit the [EnterpriseDB website](https://www.enterprisedb.com/).
> 
...
Before installing Advanced Server and EDB Replicate, use yum to install prerequisite packages:
...
**Step 1:** You must register with EnterpriseDB to receive credentials for the EDB repository. If you have not previously registered with EnterpriseDB, visit the [EnterpriseDB site for information about requesting credentials](https://www.enterprisedb.com/repository-access).

**Step 2:** Install the EnterpriseDB repository configuration package:

``` text
yum -y install https://yum.enterprisedb.com/edb-repo-rpms/edb-repo-latest.noarch.rpm
```

**Step 3:** With your choice of editor, edit the `/etc/yum.repos.d/edb.repo` file, replacing the `username` and `password` placeholders in the baseurl specification with the name and password of a registered EnterpriseDB user and ensure that the `gpgcheck` and `enabled` properties are set to `1`.

**Step 4:** Update the yum cache and install Advanced Server:
...
**Step 5:** Use sudo to assume the identity of the `enterprisedb` database superuser and create an Advanced Server cluster on listener port `5444`:
...
``` text
/usr/edb/as12/bin/psql -d edb -p 5444
```
...
You can use simple SQL commands to query the database:
...
For detailed product usage information, please see the [Advanced Server documentation, available at the EnterpriseDB website](https://www.enterprisedb.com/edb-docs/).
