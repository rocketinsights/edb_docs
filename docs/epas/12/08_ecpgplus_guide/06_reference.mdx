...
Your application must allocate the space required to hold each value. Refer to [displayResultSet](04_building_executing_dynamic_sql_statements#executing_query_with_unknown_number_of_variables) function for an example of how to allocate space for `SELECT`-list values.
