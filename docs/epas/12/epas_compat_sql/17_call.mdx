...
> </div>
> 
> You must specify an `OUT` parameter in the `CALL` statement when calling a package function; the `OUT` parameter will act as an `INOUT` parameter during package overloading.
> 
> </div>
