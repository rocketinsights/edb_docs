...
> Include the `-k` or `--link` keyword to create a hard link from the new cluster to the old cluster. See [Linking versus Copying](../../04_upgrading_an_installation_with_pg_upgrade/01_performing_an_upgrade/#linking_versus_copying) for more information about using a symbolic link.
...
> </div>
> 
> If the original Advanced Server installation is using port number `5444` when you invoke the Advanced Server 13 installer, the installer will recommend using listener port `5445` for the new installation of Advanced Server.
> 
> </div>
