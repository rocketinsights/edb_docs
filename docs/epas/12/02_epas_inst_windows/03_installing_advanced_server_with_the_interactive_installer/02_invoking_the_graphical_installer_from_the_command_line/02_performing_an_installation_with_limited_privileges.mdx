...
``` text
Please enter the installation directory [ C:\Program Files\edb\as13 ] :
```

Specify the directory in which Advanced Server data will be stored:

``` text
Please enter the data directory path: [C:\Program Files\edb\as13\data ] :
```
...
Specify `postgresql` to install Advanced Server with complete compatibility with Postgres version 13.
...
``` text
NOTE: We will not be able to examine, if port is currently used by other
application.
