...
The `Services` window displays an alphabetized list of services; the `edb-as-12` service controls Advanced Server.
  ...
  - Use the `Pause the service` option to tell Advanced Server to reload the server configuration parameters without disrupting user sessions for many of the configuration parameters. See [Configuring Advanced Server](../04_configuring_advanced_server/index#configuring_advanced_server) for more information about the parameters that can be updated with a server reload.
