---
title: "pg\_prewarm.autoprewarm"
---

<div id="pg_prewarm.autoprewarm" class="registered_link"></div>
...
Once the `autoprewarm` process has finished loading buffers from disk, it will periodically dump shared buffers to disk at the interval specified by the `pg_prewarm.autoprewarm_interval` parameter. See the <span class="title-ref">pg_prewarm.autoprewarm_interval \<pg_prewarm.autoprewarm_interval\></span> section for information on the `autoprewarm` background worker. Upon the next server restart, the `autoprewarm` process will prewarm shared buffers with the blocks that were last dumped to disk.
