...
This guide describes the features of EnterpriseDB’s *Postgres Plus Enterprise Edition* product. The core of Postgres Plus Enterprise Edition is EnterpriseDB’s database server, *Postgres Plus Advanced Server (version 9.4)*.

Enterprise Edition provides a wide range of additional functionality in various areas including database administration, enhanced SQL capabilities, database and application security, performance monitoring and analysis, and application development utilities.
...
>
> *Open Client Library* described in Section <span class="underline">7</span> provides another application programming interface for Advanced Server.
>
> *Debugger* described in Section <span class="underline">7.5</span> is a graphically oriented debugging tool for PL/pgSQL.
...
The following features have been added to Postgres Plus Advanced Server 9.3 to create Postgres Plus Advanced Server 9.4:

  - Infinite Cache has been deprecated and may be removed in a future release. Please contact your EnterpriseDB Account Manager or [mailto:sales@enterprisedb.com](mailto:sales@enterprisedb.com) for more information.

  - Advanced Server now supported the edb\_redwood\_raw\_names configuration parameter to control whether database object names appear in uppercase or lowercase letters when viewed from Redwood system catalogs. For more information, see Section <span class="underline">2.1.3.11.3</span>.

  - Advanced Server now supports the Unicode Collation Algorithm for defining alternate collation sequences. For more information, see Section <span class="underline">2.3</span>.

  - Advanced Server now supports the CONNECT\_BY\_ROOT operator for hierarchical queries. For more information, see Section <span class="underline">3.2.6</span>.

  - Advanced Server now supports an IMMUTABLE form of the TO\_CHAR(TIMESTAMP, *format*) function, which allows creation of an index using such function. For more information, see Section <span class="underline">3.3.7.1</span>.

  - Advanced Server now supports functions using the Uniform Resource Locator. For more information, see Section <span class="underline">3.3.13</span>.

  - Advanced Server now supports HASH partitioned tables. For more information about the table partitioning syntax supported, see Section <span class="underline"> </span> <span class="underline">3.4</span>.

  - Advanced Server now supports more comprehensive logging of SQL injection attack queries by SQL/Protect. For more information, see Section <span class="underline">4.1.1.2.3</span>.

  - Advanced Server now supports the EDB Resource Manager for controlling system usage by Advanced Server processes. For more information, see Chapter <span class="underline">5</span>.

  - Advanced Server now supports the utl\_file.umask configuration parameter to set file permissions for the UTL\_FILE package. For more information, see Section <span class="underline">9.16.1</span>.

  - Advanced Server now supports additional functionality for the UTL\_HTTP package. For more information, see Section <span class="underline">9.17</span>.

  - Advanced Server now supports additional functionality for EDB\*Loader including the use of environment variables in the control file for directory paths and file names, the PRESERVE BLANKS clause for preserving leading and trailing white space, the DISCARDMAX and DISCARDS clauses for limiting the number of discarded, input data records, the DATA command line parameter, data conversion using the CHARACTERSET clause for loading data in a different character set encoding than the target database character set encoding, and assignment of an EDB\*Loader session to an EDB Resource Manager resource group. For more information, see Section <span class="underline">6.1</span>.

  - Advanced Server now includes the edb\_custom\_plan\_tries configuration parameter. For more information, see Section <span class="underline">2.1.3.5.1</span>.

  - Advanced Server now supports the EDB\_ATTR\_STMT\_LEVEL\_TX server attribute. For more information, see [Section 7.4.2.2](#ocl-function-reference).

  - Advanced Server now supports Oracle-compatible LOB functions. For more information, see [Section 7.4.11](#_LOB_Functions).

  - Advanced Server now supports OCIPGErrorGet. For more information, see [Section 7.4.12](#_Miscellaneous_Functions).

  - Advanced Server now supports WITH HOLD cursors. For more information, see Section <span class="underline">7.4.2.1</span>.

  - The edb\_get\_server\_processes() and edb\_get\_process\_utilizations() functions have been deprecated in Advanced Server version 9.4 and will be removed in version 9.5.
...
Throughout this guide, the directory path of Postgres Plus Advanced Server is referred to as *POSTGRES\_PLUS\_HOME*.
