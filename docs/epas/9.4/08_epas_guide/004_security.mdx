...
**Note:** SQL/Protect statistics are maintained in memory while the database server is running. When the database server is shut down, the statistics are saved to a binary file named edb\_sqlprotect.stat in the data/global subdirectory of the Postgres Plus Advanced Server home directory.
...
**Step 1:** Edit the following configuration parameters in the postgresql.conf file located in the data subdirectory of your Postgres Plus Advanced Server home directory.
 ...
 - **edb\_sql\_protect.max\_protected\_relations.** Sets the maximum number of relations that can be protected per role. If this parameter is omitted, the default setting is 1024. See Section <span class="underline">2.1.3.12.7</span> for information on the maximum range of this parameter.
...
**For Linux only:** Run the /etc/init.d/ppas-9.4 script with the restart option as shown by the following:
...
**For Windows only:** Open Control Panel, Administrative Tools, and then Services. Restart the service named ppas-9.4.

![](./images/image4.jpeg)

*Figure 4.2 - Starting the ppas-9.4 service.*
...
$ /opt/PostgresPlus/9.4AS/bin/psql -d edb -U enterprisedb
...
edb=\# \\i /opt/PostgresPlus/9.4AS/share/contrib/sqlprotect.sql
...
The enterprisedb operating system account (postgres if you installed Postgres Plus Advanced Server in PostgreSQL compatibility mode) must have read and write access to the directory specified in *sqlprotect\_file*.
...
$ /opt/PostgresPlus/9.4AS/bin/psql -d newdb -U enterprisedb -f /tmp/edb.dmp
...
$ /opt/PostgresPlus/9.4AS/bin/psql -d newdb -U enterprisedb
...
EDB\*Wrap Utility: Release 9.4.0.0
