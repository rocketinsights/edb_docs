---
title: "Upgrading an Installation With pg\_upgrade"
---
...
Without pg\_upgrade, to migrate from an earlier version of Advanced Server to Advanced Server 9.4, you must export all of your data using pg\_dump, install the new release, run initdb to create a new cluster, and then import your old data. If you have a significant amount of data, that can take a considerable amount of time and planning. You may also have to use additional storage to temporarily accommodate both the original data and the exported data.
...
Several factors determine if an in-place upgrade is practical:
 ...
 - To upgrade between major versions of Advanced Server with pg\_upgrade, both versions must share a common binary representation for each data type. Therefore, you cannot use pg\_upgrade to migrate between 32-bit and 64-bit Linux platforms.
...
The pg\_upgrade executable is distributed with Postgres Plus Advanced Server 9.4, and is installed as part of the Database Server component; no additional installation or configuration steps are required.
...
> pg\_upgrade --old-datadir *path\_to\_9.3\_data\_directory* --new-datadir *path\_to\_9.4\_data\_directory* --user *superuser\_name* --old-bindir *path\_to\_9.3\_bin\_directory* --new-bindir *path\_to\_9.4\_bin\_directory* --old-port *9.3\_port* --new-port *9.4\_port*

Where:

\--old-datadir *path\_to\_9.3\_data\_directory*
...
\--new-datadir *path\_to\_9.4\_data\_directory*
...
>
> Please note: If the original Advanced Server installation is using port number 5444 when you invoke the Advanced Server 9.4 installer, the installer will recommend using listener port 5445 for the new installation of Advanced Server.
...
## Upgrading to Advanced Server 9.4

You can use pg\_upgrade to upgrade from an existing installation of Advanced Server into the cluster built by the Advanced Server 9.4 installer or into an alternate cluster created using the initdb command. In this section, we will provide the details of upgrading into the cluster provided by the installer.

The basic steps to perform an upgrade into an empty cluster created with the initdb command are the same as the steps to upgrade into the cluster created by the Advanced Server 9.4 installer, but you can omit Step 2 (*Empty the edb database*), and substitute the location of the alternate cluster when specifying a target cluster for the upgrade.
...
Install Advanced Server 9.4, specifying the same non-server components that were installed during the previous Advanced Server installation. Please note that the new cluster and the old cluster must reside in different directories.

Step 2 - Empty the target database

The target cluster must not contain any data; you can create an empty cluster using the initdb command, or you can empty a database that was created during the installation of Advanced Server 9.4. If you have installed Advanced Server in PostgreSQL mode, the installer creates a single database named postgres; if you have installed Advanced Server in Oracle mode, it creates a database named postgres and a database named edb.
...
By default, the Advanced Server installation process installs and starts the following services:
 ...
 - ppas-agent-9.4 - The PgAgent service
...
During the upgrade process, pg\_upgrade will connect to the old and new servers several times; to make the connection process easier, you should edit the pg\_hba.conf file, setting the authentication mode to trust. To modify the pg\_hba.conf file, navigate through the Start menu to each Postgres Plus Advanced Server menu, and open the Expert Configuration menu; select the Edit pg\_hba.conf menu option to open the pg\_hba.conf file.

You should allow trust authentication for the previous Advanced Server installation, and Advanced Server 9.4 servers. Edit the pg\_hba.conf file for both installations of Advanced Server as shown in Figure 8.1.
...
Before invoking pg\_upgrade, you must stop any services that belong to the original Advanced Server installation, Advanced Server 9.4 or the supporting components. This ensures that a service will not attempt to access either cluster during the upgrade process.
...
Open a terminal window, assume superuser privileges, and navigate to the /etc/init.d directory. Manually stop each Advanced Server service at the command line; for example, to stop Advanced Server 9.4 invoke the command:

> ./ppas-9.4 stop

Step 5 - Assume the identity of the cluster owner

Assume the identity of the Advanced Server cluster owner, and add the directory in which the pg\_upgrade executable resides to the search path.
...
> RUNAS /USER:enterprisedb "CMD.EXE"
> SET PATH=%PATH%;C:\\Program Files\\PostgresPlus\\9.4AS\\bin
...
During the upgrade process, pg\_upgrade writes a file to the current working directory of the enterprisedb user; you must invoke pg\_upgrade from a directory where the enterprisedb super user has write privileges. After performing the above commands, navigate to a directory in which the enterprisedb user has sufficient privileges to write a file.
...
> pg\_upgrade -d /opt/PostgresPlus/9.3AS/data -D /opt/PostgresPlus/9.4AS/data -U enterprisedb -b /opt/PostgresPlus/9.3AS/bin -B /opt/PostgresPlus/9.4AS/bin -p 5444 -P 5445 --check

If you are using Windows, you must quote any directory names that contain a space:

> pg\_upgrade.exe -d "C:\\Program Files\\PostgresPlus\\9.3AS\\data"
> \-D "C:\\Program Files\\PostgresPlus\\9.4AS\\data" -U enterprisedb
> \-b "C:\\Program Files\\PostgresPlus\\9.3AS\\bin" -B "C:\\Program Files\\PostgresPlus\\9.4AS\\bin" -p 5444 -P 5445 --check
...
If pg\_upgrade alerts you to a missing component, you can use StackBuilder Plus to add the component that contains the component. Before using StackBuilder Plus, you must restart the Advanced Server 9.4 service. After restarting the service, open StackBuilder Plus by navigating through the Start menu to the Postgres Plus Advanced Server 9.4 menu, and selecting StackBuilder Plus. Follow the onscreen advice of the StackBuilder Plus wizard to download and install the missing components.

For more information about using StackBuilder Plus, please see [Chapter 4.5](#_Using_StackBuilder_Plus_13), *Using StackBuilder Plus*.
...
> pg\_upgrade -d /opt/PostgresPlus/9.3AS/data -D /opt/PostgresPlus/9.4AS/data -U enterprisedb -b /opt/PostgresPlus/9.3AS/bin -B /opt/PostgresPlus/9.4AS/bin -p 5444 -P 5445

On Windows:

> pg\_upgrade.exe -d "C:\\Program Files\\PostgresPlus\\9.3AS\\data"
> \-D "C:\\Program Files\\PostgresPlus\\9.4AS\\data" -U enterprisedb
> \-b "C:\\Program Files\\PostgresPlus\\9.3AS\\bin" -B "C:\\Program Files\\PostgresPlus\\9.4AS\\bin" -p 5444 -P 5445
...
Rename $PGDATA/global/pg\_control, removing the .old suffix.
...
If pg\_upgrade reports the error message shown in Figure 8.2, please stop all Advanced Server services and try the upgrade again. See Step 4 in [Section 8.3](#upgrading-to-advanced-server-9.4), *Upgrading to Advanced Server 9.4* for detailed information about stopping a service.

!\[image\]\(./images/image89.png)

*Figure 8.2 — There seems to be a postmaster servicing the new cluster*

### Upgrade Error - fe\_sendauth: no password supplied

If pg\_upgrade reports the error shown in Figure 8.3, please modify the pg\_hba.conf files in the old and new cluster to enable trust authentication, or configure the system to use a pgpass.conf file. See Step 3 in [Section 8.3](#upgrading-to-advanced-server-9.4), *Upgrading to Advanced Server 9.4* for detailed information about editing the pg\_hba.conf file.

!\[image\]\(./images/image90.png)

*Figure 8.3 — fe\_sendauth: no password supplied*

### Upgrade Error - New cluster is not empty; exiting

If pg\_upgrade reports the error shown in Figure 8.4, please empty the new cluster; see Step 2 in [Section 8.3](#upgrading-to-advanced-server-9.4), *Upgrading to Advanced Server 9.4* for more information about providing an empty target cluster. **Please note:** the target cluster may not contain any user-defined databases.

!\[image\]\(./images/image91.png)

*Figure 8.4 — New cluster is not empty; exiting*

### Upgrade Error - Failed to load library

If the original Advanced Server cluster included libraries that are not included in the Advanced Server 9.4 cluster, pg\_upgrade will alert you to the missing component during the consistency check by writing an entry to the loadable\_libraries.txt file in the directory from which you invoked pg\_upgrade (as shown in Figure 8.5).

![](./images/image92.png)

*Figure 8.5 — Failed to load library*
...
Please Note: Advanced Server (version 9.2 and later) is compatible with PostGIS 2.0. For detailed instructions about upgrading to PostGIS 2.0, see [Section 8.4.5](#reverting-to-the-old-cluster).
