...
*variable\_declaration* is a declaration of a variable, which can be accessed only from within the subprogram. There can be none, one, or more variable declarations. *statement* is an SPL program statement.
...
Object type emp\_obj\_typ contains a member method named display\_emp. display\_emp uses a SELF parameter, that passes the object instance on which the method is invoked.
