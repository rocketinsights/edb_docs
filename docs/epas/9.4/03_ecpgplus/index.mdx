...
EnterpriseDB has enhanced ECPG (the PostgreSQL pre-compiler) to create ECPGPlus. ECPGPlus allows you to include Pro\*C compatible embedded SQL commands in C applications when connected to an Advanced Server database. When you use ECPGPlus to compile an application, the SQL code is syntax-checked and translated into C.
...
While most ECPGPlus statements will work with community PostgreSQL, the CALL statement, and the EXECUTE…END EXEC statement work only when the client application is connected to Postgres Plus Advanced Server.
