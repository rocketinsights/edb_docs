...
The `ENCRYPT` function or procedure uses a user-specified algorithm,
key, and optional initialization vector to encrypt `RAW`, `BLOB` or
`CLOB` data. The signature of the `ENCRYPT` function is:
...
When invoked as a procedure, `ENCRYPT` returns `BLOB` or `CLOB` data to
a user-specified `BLOB`.
...
> `<typ>` specifies the block cipher type that will be used by
> `ENCRYPT`, and any modifiers. Advanced Server supports the block
> cipher algorithms, modifiers and cipher suites listed below:
...
> `<iv>` (optional) specifies an initialization vector. By default,
> `<iv>` is `NULL`.
...
`ENCRYPT` uses a key value of `my secret key` and an initialization
vector of `my initialization vector` when encrypting the `password`;
specify the same key and initialization vector when decrypting the
`password`.
