...
value of a `RAW` or `CLOB` value. The `HASH` function is available in
three forms:
...
> `<typ>` specifies the `HASH` function type. Advanced Server supports
> the `HASH` function types listed below:
...
The following example uses `DBMS_CRYPTO.HASH` to find the `md5` hash
