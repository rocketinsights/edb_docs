...
Now create a data redaction policy on employees to redact the `ssn` column which should be accessible in equality condition and salary with default scope and exception. The redaction policy will be exempt for the `hr` user.
