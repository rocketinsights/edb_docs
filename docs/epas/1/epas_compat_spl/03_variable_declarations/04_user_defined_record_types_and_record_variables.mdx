...
Records can be declared based upon a table definition using the `%ROWTYPE` attribute as shown in [Using %ROWTYPE in Record Declarations](using_%_row_type_in_record_declarations). This section describes how a new record structure can be defined that is not tied to any particular table definition.
