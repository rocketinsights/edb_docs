...
\- The `edb_stmt_level_tx` parameter must be set to `TRUE`. This prevents the action of unconditionally rolling back all database updates within the `BEGIN/END` block if any exception occurs. See [edb_stmt_level_tx](edb_stmt_level_tx) for more information on the `edb_stmt_level_tx` parameter.
