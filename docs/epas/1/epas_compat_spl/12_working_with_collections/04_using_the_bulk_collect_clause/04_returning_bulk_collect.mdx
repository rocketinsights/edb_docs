...
The `BULK COLLECT` clause can be added to the `RETURNING INTO` clause of a `DELETE, INSERT`, or `UPDATE` command. (See [Using the RETURNING INTO Clause](using_the_returning_into_clause) for information on the `RETURNING INTO` clause.)
...
`insert, update`, and `delete` are the `INSERT, UPDATE`, and `DELETE` commands as described in [INSERT](insert),
