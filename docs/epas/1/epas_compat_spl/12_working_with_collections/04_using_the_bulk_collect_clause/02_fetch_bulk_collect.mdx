...
The `BULK COLLECT` clause can be used with a `FETCH` statement. (See [Fetching Rows From a Cursor](fetching_rows_from_a_cursor) for information on the `FETCH` statement.) Instead of returning a single row at a time from the result set, the `FETCH BULK COLLECT` will return all rows at once from the result set into the specified collection unless restricted by the `LIMIT` clause.
