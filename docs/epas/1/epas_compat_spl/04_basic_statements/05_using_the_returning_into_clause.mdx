...
[Using the BULK COLLECT Clause](using_the_bulk_collect_clause) for more information on the `BULK COLLECT` clause.

The following example is a modification of the `emp_comp_update` procedure introduced in [UPDATE](update), with the addition of the `RETURNING INTO` clause.
