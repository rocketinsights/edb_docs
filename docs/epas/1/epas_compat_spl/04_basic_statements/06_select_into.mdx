...
[SELECT BULK COLLECT](select_bulk_collect) for more information on using the `BULK COLLECT` clause with the `SELECT INTO` statement.
...
[Exception Handling](exception_handling) for more information on exception handling.
