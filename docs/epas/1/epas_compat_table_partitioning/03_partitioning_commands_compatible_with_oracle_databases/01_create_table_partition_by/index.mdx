...
Use the `INTERVAL` clause to specify an interval partitioned table. By specifying an `INTERVAL` clause, the range partitioning is extended by the database automatically to create partitions of a specified interval when new data is inserted into a table that exceeds an existing range partition.

For more information about `INTERVAL PARTITION`, see [Interval Partitioning](interval_partitioning).
...
example_partition_by_list example_partition_by_range example_interval_partitioning example_partition_by_hash example_partition_by_range_subpartition_by_list
