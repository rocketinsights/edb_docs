...
To create a database cluster in the `PGDATA` directory that listens on the port specified by the `PGPORT` environment variable specified in the service configuration file (described in [Using a Service Configuration File on CentOS or Redhat 6.x](using_a_service_configuration_file_on_centos_or_redhat_6x)), assume `root`
...
[Specifying Cluster Options with INITDBOPTS](specifying_cluster_options_with_initdbopts).
