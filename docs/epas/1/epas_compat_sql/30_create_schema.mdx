...
`CREATE SCHEMA` includes subcommands to create objects within the
schema. The subcommands are treated essentially the same as separate
commands issued after creating the schema. All the created objects will
be owned by the specified user.
...
> role, `username`, must already exist, but the schema must not exist.
> In Oracle, the user (equivalently, the schema) must exist.)

`schema_element`

> An SQL statement defining an object to be created within the schema.
> `CREATE TABLE`, `CREATE VIEW`, and `GRANT` are accepted as clauses
