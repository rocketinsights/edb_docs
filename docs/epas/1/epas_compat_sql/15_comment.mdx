...
issue a new `COMMENT` command for the same object. Only one comment
string is stored for each object. To remove a comment, specify the empty
string (two consecutive single quotes with no intervening space) for
`text`. Comments are automatically dropped when the object is dropped.
...
> The name of a column within `table_name` to be commented. The table
