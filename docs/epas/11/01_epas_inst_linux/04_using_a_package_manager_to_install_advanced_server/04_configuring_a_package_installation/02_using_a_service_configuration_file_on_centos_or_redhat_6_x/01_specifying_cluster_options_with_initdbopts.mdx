...
<div
  id="specifying_cluster_options_with_initdbopts"
...
> Include the `--icu-short-form` keywords to create a cluster that uses a default ICU (International Components for Unicode) collation for all databases in the cluster. For more information about Unicode collations, please refer to the _EDB Postgres Advanced Server Guide_ available at:
>
...
> /_`path_to_initdb_installation_directory`_/`initdb --help`

Where _`path_to_initdb_installation_directory`_ specifies the location of the `initdb` binary file.
