...
The following sections discuss the features and functionality of the Debugger using the standalone debugging method. The directions for starting the Debugger for in-context debugging are discussed in the [Setting Global Breakpoint for In-Context Debugging ](11_debugger#setting_global_breakpoint_for_in_context_debugging) section.
...
Note that triggers cannot be debugged using standalone debugging. Triggers must be debugged using in-context debugging. See the [Setting Global Breakpoint for In-Context Debugging ](11_debugger#setting_global_breakpoint_for_in_context_debugging) section for information on setting a global breakpoint for in-context debugging.
...
![*Setting a global breakpoint from the Object men*](images/setting_global_breakpoint_from_object_menu.png)
