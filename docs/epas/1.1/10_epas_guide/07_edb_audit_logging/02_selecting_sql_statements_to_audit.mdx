...
> `ACCESS METHOD` `AGGREGATE` `CAST` `COLLATION` `CONVERSION` `DATABASE` `EVENT TRIGGER` `EXTENSION` `FOREIGN TABLE` `FUNCTION` `INDEX` `LANGUAGE` `LARGE OBJECT` `MATERIALIZED VIEW` `OPERATOR` `OPERATOR CLASS` `OPERATOR FAMILY` `POLICY` `PUBLICATION` `ROLE` `RULE` `SCHEMA` `SEQUENCE` `SERVER` `SUBSCRIPTION` `TABLE` `TABLESPACE` `TEXT SEARCH CONFIGURATION` `TEXT SEARCH DICTIONARY` `TEXT SEARCH PARSER` `TEXT SEARCH TEMPLATE` `TRANSFORM` `TRIGGER` `TYPE` `USER MAPPING` `VIEW`
