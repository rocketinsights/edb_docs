...
The following changes have been made to EDB Postgres Failover Manager to create version 3.9:
  ...
  - The [use.replay.tiebreaker ](04_configuring_efm/01_cluster_properties/index#use.replay.tiebreaker) property specifies how Failover Manager determines which standby node will assume the role of master if two standby nodes have received the same amount of data.

  - The [reconfigure.num.sync ](04_configuring_efm/01_cluster_properties/index#reconfigure.num.sync) property specifies if Failover Manager will adjust the synchronous_standby_names parameter on the master node.
