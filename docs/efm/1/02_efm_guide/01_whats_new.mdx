...
\- The [use.replay.tiebreaker](use.replay.tiebreaker) property specifies how Failover Manager determines which standby node will assume the role of master if two standby nodes have received the same amount of data.

\- The [reconfigure.num.sync](reconfigure.num.sync) property specifies if Failover Manager will adjust the synchronous_standby_names parameter on the master node.
