...
1.  Unless `auto.allow.hosts` is set to `true`, use the `efm allow-node` command, to add the IP address of the new node to the Failover Manager allowed node host list. When invoking the command, specify the cluster name and the IP address of the new node:
    ...
    Install a Failover Manager agent and configure the cluster properties file on the new node. For more information about modifying the properties file, see [The Cluster Properties File ](04_configuring_efm/01_cluster_properties/index#cluster_properties).
...
Please note that stopping an agent does not signal the cluster that the agent has failed unless the [master.shutdown.as.failure ](04_configuring_efm/01_cluster_properties/index#master_shutdown_as_failure) property is set to `true`.
...
After creating the `acctg.properties` and `sales.properties` files, create a service script or unit file for each cluster that points to the respective property files; this step is platform specific. If you are using RHEL 6.x or CentOS 6.x, see [RHEL 6.x or CentOS 6.x ](05_using_efm#rhel_or_centos_6); if you are using RHEL 7.x or CentOS 7.x, see [RHEL 7.x or CentOS 7.x ](05_using_efm#rhel_or_centos_7).
