...
After invoking the `efm disallow-node` command, you must use the <span class="title-ref">efm allow-node \<efm_allow_node\></span> command to add the node to the cluster again.
...
After creating the `acctg.properties` and `sales.properties` files, create a service script or unit file for each cluster that points to the respective property files; this step is platform specific. If you are using RHEL 6.x or CentOS 6.x, see [RHEL 6.x or CentOS 6.x](#rhel_or_centos_6); if you are using RHEL/CentOS 7.x or RHEL/CentOS 8.x, see <span class="title-ref">RHEL/CentOS 7.x or RHEL/CentOS 8.x \<rhel_or_centos_7\></span>.
