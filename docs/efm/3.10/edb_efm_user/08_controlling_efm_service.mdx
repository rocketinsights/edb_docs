\<div id="controlling\_efm\_service" class="registered\_link"\>\</div\>
...
\<div id="service\_utility" class="registered\_link"\>\</div\>
...
\<div id="systemctl" class="registered\_link"\>\</div\>
...
On RHEL/CentOS 7.x and RHEL/CentOS 8.x, runs as a Linux service named (by default) `edb-efm-3.10.service` that is located in `/usr/lib/systemd/system`. Each database cluster monitored by Failover Manager will run a copy of the service on each node of the replication cluster.
