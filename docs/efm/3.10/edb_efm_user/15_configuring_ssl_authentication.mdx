\<div id="configuring\_ssl\_authentication" class="registered\_link"\>\</div\>
...
3.  Modify the `pg_hba.conf` file on each node of the cluster, adding the following line to the beginning of the file:
    ...
    The line instructs the server to reject any connections that are not using SSL authentication; this enforces SSL authentication for any connecting clients.  For information about modifying the pg\_hba.conf file, see the PostgreSQL core documentation at:
...
6.  Modify the [efm.properties file](#jdbc.sslmode) on each node within the cluster, setting the `jdbc.sslmode` property.
