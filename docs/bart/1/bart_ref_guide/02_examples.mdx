...
> > <https://www.enterprisedb.com/edb-docs/>
  ...
  - [Using a Redundancy Retention Policy ](02_examples#redundancy_retention_policy) provides an example of using the `MANAGE` and `DELETE` subcommands when a 3 backup redundancy retention policy is in effect.
  - [Using a Recovery Window Retention Policy ](02_examples#recovery_window_retention_policy) provides an example of using the `MANAGE` and `DELETE` subcommands when a 1-day recovery window retention policy is in effect.
