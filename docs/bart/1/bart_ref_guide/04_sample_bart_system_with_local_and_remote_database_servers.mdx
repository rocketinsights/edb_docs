...
For detailed information about configuring a BART system, see the *EDB Postgres Backup and Recovery Installation and Upgrade Guide* and for information about the operational procedures and BART subcommands, see the *EDB Postgres Backup and Recovery User Guide* available at:
...
The following sections demonstrate configuring and taking full backups only. To support incremental backups as well, enable the `allow_incremental_backups` parameter for the desired database servers and use the `WAL scanner` program.
  ...
  - [The BART Configuration File ](04_sample_bart_system_with_local_and_remote_database_servers#bart_configuration_file) shows the settings used in the BART configuration file.
  - `Establishing SSH/SCP Passwordless Connections <establishing_ssh/scp_passwordless_connections>` provides an example of how to establish an SSH/SCP passwordless connection.
  - [Configuring a Replication Database User ](04_sample_bart_system_with_local_and_remote_database_servers#configuring_a_replication_database_user) provides an example of how to configure the replication database user.
  - [WAL Archiving Configuration Parameters ](04_sample_bart_system_with_local_and_remote_database_servers#wal_archiving_configuration_parameters) provides an example of how to configure WAL archiving.
  - [Creating the BART Backup Catalog ](04_sample_bart_system_with_local_and_remote_database_servers#creating_the_bart_backup_catalog_backup_path) provides information about creating a BART Backup Catalog.
  - [Starting the Database Servers with WAL Archiving ](04_sample_bart_system_with_local_and_remote_database_servers#starting_the_database_servers_with_wal_archiving) provides example of starting the database servers with WAL archiving.
  - [Taking a Full Backup ](#taking_a_full_backup) illustrates taking the first full backup of the database servers.
  - [Using Point-In-Time Recovery ](04_sample_bart_system_with_local_and_remote_database_servers#using_point-in-time_recovery) demonstrates the point-in-time recovery operation on the remote PostgreSQL database server.
...
<div id="establishing_ssh/scp_passwordless_connections" class="registered_link"></div>
...
This section shows how the passwordless SSH/SCP connections are established with the authorized public keys files.
...
To generate the public key file, first create the `.ssh` subdirectory in the BART user’s home directory:
...
On the BART host where the public key file was just generated as shown in [Generating a Public Key File for the BART User Account ](04_sample_bart_system_with_local_and_remote_database_servers#generating_a_public_key_file_for_the_bart_user_account), create the authorized keys file by appending the public key file to any existing authorized keys file.
...
The public key file, `id_rsa.pub`, for BART user account `enterprisedb` on the BART host was generated in [Generating a Public Key File for the BART User Account ](04_sample_bart_system_with_local_and_remote_database_servers#generating_a_public_key_file_for_the_bart_user_account) Section, and is now copied to the remote Advanced Server host on `192.168.2.24`:
...
The public key file, `id_rsa.pub`, for BART user account `enterprisedb` on the BART host was generated in [Generating a Public Key File for the BART User Account ](04_sample_bart_system_with_local_and_remote_database_servers#generating_a_public_key_file_for_the_bart_user_account) Section, and is now copied to the remote PostgreSQL host on 192.168.2.24:
...
For more information about using a `.pgpass` file, please see the PostgreSQL documentation available at:
...
If the Postgres `archive_command` is already set, to prevent the `archive_command` from re-setting invoke the [INIT` subcommand with the `-- no-configure` option. For details, see [INIT ](01_using_subcommands/04_init#init) section.
...
Before invoking any BART subcommands, set up a profile under the BART user account’s home directory to set the `LD_LIBRARY_PATH` and `PATH` environment variables.

</div>

For more information regarding setting this variable, see the Configuring the BART host section of *EDB Postgres Backup and Recovery Installation and Upgrade Guide* available at:
...
After the BART backup catalog directory structure has been completed, begin the archiving of WAL files from the database servers by restarting each database server. On BART host 192.168.2.22:
...
The following lists the current PostgreSQL server WAL files. The unarchived WAL files are marked with two stars (**).
