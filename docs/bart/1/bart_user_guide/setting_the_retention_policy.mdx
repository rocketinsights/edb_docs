...
metaTitle: Setting the Retention Policy
metaDescription: description
---

The retention policy is determined by the `retention_policy` parameter
in the BART configuration file. It can be applied globally to all
servers, but each server can override the global retention policy with
its own. For information about creating a global retention policy and an
...
max_number BACKUPS` where `max_number` is a positive integer designating
the maximum number of most recent backups.
...
The redundancy retention policy is the default type of retention policy
if all keywords `BACKUPS`, `DAYS`, `WEEKS`, and `MONTHS` following the
`max_number` integer are omitted as shown by the following example:
...
The `SHOW-SERVERS` subcommand displays the `3 Backups` redundancy
retention policy in the `RETENTION POLICY` field:
...
**Additional Restrictions:**
  ...
  - The keywords `DAYS`, `WEEKS`, and `MONTHS` must always be specified
    in plural form (for example, `1 DAYS`, `1 WEEKS`, or `1 MONTHS`).
  - BART will accept a maximum integer value of `2,147,483,647` for
