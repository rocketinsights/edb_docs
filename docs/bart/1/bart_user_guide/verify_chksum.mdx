...
The `VERIFY-CHKSUM` subcommand is only used for tar format backups. It
is not applicable to plain format backups.
...
> tablespaces.
> 
> `<backup_name>` is the user-defined alphanumeric name for the full
> backup.
> 
> If `all` is specified or if the `-i` option is omitted, the checksums
> of all tar backups for the relevant database server are verified.
