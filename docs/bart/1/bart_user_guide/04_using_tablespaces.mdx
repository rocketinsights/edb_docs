...
tablespaces created with the `CREATE TABLESPACE` command):
  ...
  - You can take full backups using the transaction log streaming method
    ...
    `--with-pg_basebackup` and the `BACKUP` subcommand in either tar
    (`-F t`) or plain text (`-F p`) backup file format.
...
If the particular database cluster you plan to back up contains
tablespaces created by the `CREATE TABLESPACE` command, then you must
set the `tablespace_path` parameter in the BART configuration file
...
Where `OID_1`, `OID_2`, … are the Object Identifiers of the tablespaces.
You can find the OIDs of the tablespaces and their corresponding soft
links to the directories by listing the contents of the
`POSTGRES_INSTALL_HOME/data/pg_tblspc` subdirectory as shown in the
following example:
...
`/mnt/tablespace_2`, respectively.
  ...
  - If you later wish to restore the tablespaces to the same locations
    as indicated in the preceding example, the BART configuration file
