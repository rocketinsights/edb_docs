...
BART takes full backups using the `pg_basebackup` utility program under the following conditions:
  ...
  - Database servers can only be backed up by using `pg_basebackup` utility program of the same or later version than the database server version. For example, `pg_basebackup` version 9.5 can back up database server version 9.5, but it cannot back up database server version 9.6.
...
For information about `pg_basebackup`, see [PostgreSQL Core Documentation](https://postgresql.org/docs/current/static/app-pgbasebackup.html).
