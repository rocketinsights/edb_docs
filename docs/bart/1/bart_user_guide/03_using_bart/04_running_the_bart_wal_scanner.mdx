...
The BART user account’s `LD_LIBRARY_PATH` environment variable may need to be set to include the directory containing the `libpq` library if invocation of the WAL scanner program fails. See [Section - Basic BART Subcommand Usage](../03_using_bart/03_basic_bart_subcommand_usage/index#basic_bart_subcommand_usage) for information about setting the `LD_LIBRARY_PATH` environment variable.
