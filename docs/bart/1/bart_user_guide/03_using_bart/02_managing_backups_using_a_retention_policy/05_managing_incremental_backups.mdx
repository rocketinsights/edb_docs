...
If the `MANAGE` command is invoked when BART is configured to use a [recovery window retention policy ](../../02_examples#recovery_window_retention_policy), the status of `active` full backups are changed to `obsolete` if the date/time of the full backup is outside of the recovery window.
