...
The retention policy is determined by the `retention_policy` parameter in the BART configuration file. It can be applied globally to all servers, but each server can override the global retention policy with its own. For information about creating a global retention policy and an individual database server retention policy, see the *Configuration* section of the *EDB Postgres Backup and Recovery Installation and Upgrade Guide* available at:
...
See the *EDB Postgres Backup and Recovery Reference Guide* for examples of how to view the actual, calculated recovery window.
