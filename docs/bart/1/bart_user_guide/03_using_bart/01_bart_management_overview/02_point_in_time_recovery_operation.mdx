...
6.  For point-in-time recovery, verify that the `postgresql.auto.conf` file created in the directory specified with the `RESTORE` subcommand’s `-p <restore_path>` option generated the correct recovery parameter settings.
...
8.  Start the database server, which will then perform the point-in-time recovery operation if recovery settings are saved in the `postgresql.auto.conf` file.

A detailed description of the `RESTORE` subcommand is available in the [Basic Bart Subcommand Usage](../../03_using_bart/03_basic_bart_subcommand_usage/#basic_bart_subcommand_usage) section.

Example of a Point-in-Time recovery operation is documented in the _EDB Postgres Backup and Recovery Reference Guide_ available at:
