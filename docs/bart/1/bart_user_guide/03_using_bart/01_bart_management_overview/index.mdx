...
3.  Start the Postgres database servers with archiving enabled. Verify that the WAL files are appearing in the `archive_path`. For more information about the `archive_path` parameter, see the Configuration section of the EDB Postgres Backup and Recovery Installation and Upgrade Guide available at:
    
    <https://www.enterprisedb.com/edb-docs/>
    
    The archiving frequency is dependent upon other `postgresql.conf` configuration parameters. Check the Postgres database server log files to ensure there are no archiving errors. Archiving should be operational before taking a backup in order to ensure that the WAL files that may be created during the backup process are archived.
