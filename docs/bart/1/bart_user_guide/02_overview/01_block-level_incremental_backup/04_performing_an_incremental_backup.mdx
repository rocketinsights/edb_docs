...
You must specify the following before taking an incremental backup:
  ...
  - `-F p` option for plain text format as incremental backup can only be taken in the plain text format.
...
See the [BACKUP ](../../03_using_bart/03_basic_bart_subcommand_usage/03_backup#backup) section for more information about using the [BACKUP` subcommand.
