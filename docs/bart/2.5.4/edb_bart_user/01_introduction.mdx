...
This guide provides the following information about using BART:
  ...
  - information about the [backup and recovery management process](#using_bart).
  - information about [using tablespaces](#using_tablespaces).

To view information about BART installation and upgrade, see the *EDB Postgres Backup and Recovery Installation and Upgrade Guide* and to view examples of BART operations and subcommands, see the *EDB Postgres Backup and Recovery Reference Guide*. These guides are available [at the EnterpriseDB website.](https://www.enterprisedb.com/edb-docs/)
...
\<div id="conventions\_used\_in\_this\_guide" class="registered\_link"\>\</div\>
...
BART takes full backups using the `pg_basebackup` utility program under the following conditions:
  ...
  - The `--with-pg_basebackup` option is specified with the `BACKUP` subcommand (see [Backup](#backup)).
...
In the global section of the BART configuration file, the `pg_basebackup_path` parameter specifies the complete directory path to the `pg_basebackup` program. For information about the `pg_basebackup_path` parameter and the `thread_count`, see the *EDB Postgres Backup and Recovery Installation and Upgrade Guide* available [at the EnterpriseDB website](https://www.enterprisedb.com/edb-docs/).
