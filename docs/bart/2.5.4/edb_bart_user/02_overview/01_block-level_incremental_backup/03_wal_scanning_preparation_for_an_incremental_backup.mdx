\<div id="wal\_scanning\_preparation\_for\_an\_incremental\_backup" class="registered\_link"\>\</div\>

# `WAL Scanning – Preparation for an Incremental Backup`
...
The MBM file is stored in the directory where archived\_wals will be stored, as specified in the `archive_path` parameter in the `bart.cfg` file. If the `archive_path` is not specified, the default `archived_wals` directory is:
...
See [Running the BART WAL Scanner](#running_the_bart_wal_scanner) for information about using the WAL scanner.
