\<div id="performing\_a\_restore\_operation" class="registered\_link"\>\</div\>
...
**Step 6:** Run the BART `RESTORE` subcommand.
  ...
  - Minimal recovery settings will be saved in the `postgresql.auto.conf` file and archive recovery will proceed only until consistency is reached, with no restoration of files from the archive. See [Restore](#restore) for detailed information about `Restore` subcommand.
...
See [Restore](#restore) for more information about using the BART `Restore` subcommand.

An example of a restore operation is documented in the [EDB Postgres Backup and Recovery Reference Guide.](https://www.enterprisedb.com/edb-docs/)
