...
Please note that before performing an incremental backup, you must take a full backup. For more details about incremental backup, refer to *Block-Level Incremental Backup* in the [EDB Postgres Backup and Recovery User Guide](https://www.enterprisedb.com/edb-docs/).

The following table describes the `BACKUP` options:

<table><thead><tr class="header"><th>Options</th><th>Description</th></tr></thead><tbody><tr class="odd"><td><code>-s</code> or <code>--server { &lt;server_name&gt; | all }</code></td><td><p>Use this option to specify the database server to be backed up.<br />
Specify <code>&lt;server_name&gt;</code> to take a backup of the database server (as specified in the BART configuration file).<br />
Specify <code>all</code> to take a backup of all servers.</p></td></tr><tr class="even"><td><code>-F</code> or <code>--format { p | t }</code></td><td><p>Use this option to specify the backup file format.<br />
...
An incremental backup can only be taken in plain text format (<code>p</code>).</p></td></tr><tr class="odd"><td><code>-z</code> or <code>--gzip</code> (applicable only for full backup and <code>tar</code> format)</td><td>Use this option to enable gzip compression of tar files using the default compression level (typically 6).</td></tr><tr class="even"><td><code>-c</code> or <code>--compress-level &lt;compression_level&gt;</code> (applicable only for full backup and tar format)</td><td>Use this option to specify the gzip compression level on the tar file output. <code>&lt;compression_level&gt;</code> is a digit from 1 through 9, with 9 being the best compression.</td></tr><tr class="odd"><td><code>--backup-name &lt;backup_name&gt;</code></td><td><p>Use this option to assign a user-defined, alphanumeric friendly name to the backup. The maximum permitted length of backup name is 49 characters.<br />
For detailed information about this parameter, see the <a href="https://www.enterprisedb.com/edb-docs/">EDB Postgres Backup and Recovery User Guide</a>.<br />
