...
This tutorial demonstrates using the `dnf` command to install the EDB Backup and Recovery Tool (BART) 2.5.4 on a RHEL or CentOS 8 host.  The tutorial assumes that the user has some knowledge of installation and system administration procedures, and administrative privileges on the host. It provides shortcuts that allow you to install and configure BART with minimal configuration settings.
...
| `pg_basebackup_path`       | `[BART]`       | Use this field to specify the path to the pg\_basebackup utility (`/usr/edb/as<xx>/bin/pg_basebackup`).                                                                                                                                                                                                           |
...
\<div id="passwordless" class="registered\_link"\>\</div\>
...
\<div id="replication" class="registered\_link"\>\</div\>
