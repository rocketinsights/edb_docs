...
This tutorial demonstrates using the `dnf` command to install the EDB Backup and Recovery Tool (BART) 2.5.5 on a RHEL/CentOS 8 host.  The tutorial assumes that the user has some knowledge of installation and system administration procedures, and administrative privileges on the host. It provides shortcuts that allow you to install and configure BART with minimal configuration settings.
