...
Use the `pg_dump` utility to create an archive that contains the
commands required to recreate a database. When invoking pg_dump,
include the -Ft flag to instruct pg_dump to format the output as a tar
file, and the -U flag to specify the name of the database superuser:
...
> host. Including `:/tmp/` at the end of this command directs `scp` to
...
pg_restore utility. On an Advanced Server host:
...
> Advanced Server cluster, the default is `enterprisedb`; on a
> PostgreSQL cluster, the default is `postgres`.
> 
...
To exit the psql client, enter `\q`; to exit the ssh session, type
`exit` and `Return`.
...
existing database into EDB Ark, please see the [PostgreSQL core
documentation](https://www.postgresql.org/docs/11/static/backup-dump.html)
.
...
see the [PostgreSQL core
documentation](https://www.postgresql.org/docs/11/static/runtime-config.html)
.
...
The overall cluster status (displayed in the top section of the
`Clusters` tab) is based on the values of the nodes within the cluster.

  - If all of the nodes within the cluster are up-to-date, the `UP`
    column displays a green checkmark.
...
You can use the `Upgrade` icon (located on the `Clusters tab
<ark_clusters_tab>` ) to access a dialog that allows you to update the
...
Select the radio button next to an option to:
  ...
  - Select `OS and DB Minor Version Upgrade` to invoke a `yum update`
    ...
    database version upgrade on each node of the cluster.
...
and click the `Upgrade` button, the Ark console will invoke the yum
update command on each node of the cluster. The `yum update` command
will update all installed packages to the most recent version available
of the same release (i.e., if you are running a 9.6 database server, yum
will update your database server to the most recent version of 9.6).
...
![*The Upgrade Cluster
dialog.*](images/db_mgmt_cluster_major_upgrade.png)
