...
If you have not purchased support, and would like to, you can view your
support options at:
...
information, see [Controlling the Database Server](controlling_server) .
...
You can include the `-d` or `--dbname` flag, followed by the database
name when invoking either client to specify the database to which the
...
The `Notification Email` field (on the `User` tab) allows you to change
the notification email associated with your user account; for more
information, see `The User Tab <ark_user_tab>` .
