.. _cluster_properties_:

************************************
`The Cluster Properties File`:index:
************************************
...
for the properties file named ``efm.properties.in`` in the
``/etc/edb/efm-3.9`` directory.

After completing the Failover Manager installation, you must make a working
copy of the template before modifying the file contents:
...
about each property, see 
`Specifying Cluster Properties <specifying_cluster_properties>`_.
...
.. _specifying_cluster_properties:
...
The ``db.user`` specified must have sufficient privileges to invoke selected
...
.. index:: db.service.name

Specify the name of the database service in the ``db.service.name`` property
...
For more information about database configuration files, visit the `PostgreSQL website <https://www.postgresql.org/docs/12/runtime-config-file-locations.html>`_
...
-  specify a custom email address, using the ``%h`` placeholder to represent
   the name of the node host (e.g., example@%h). The placeholder will be
   replaced with the name of the host as returned by the Linux hostname
   utility.

For more information about notifications, see :doc:`Notifications <notifications>` .
...
please see :doc:`Notifications <notifications>`.
...
.. index:: script.notification.property
...
Use the ``admin.port`` property to specify a port on which Failover Manager
...
Set the ``is.witness`` property to true to indicate that the current node is
...
.. note:: If ``is.witness`` is ``true``, Failover Manager will not check the recovery state.
...
* The ``local.period`` property specifies how many seconds between attempts
  to contact the database server.

* The ``local.timeout property`` specifies how long an agent will wait for a
  positive response from the local database server.

* The ``local.timeout.final`` property specifies how long an agent will wait
  after the final attempt to contact the database server on the current
  node. If a response is not received from the database within the
  number of seconds specified by the local.timeout.final property, the
  database is assumed to have failed.
...
.. _master_shutdown_as_failure:
.. index:: master.shutdown.as.failure
...
``master.shutdown.as.failure`` is ``true``, you should stop the master agent and
...
Alternatively, you can use the ``efm stop-cluster`` command to stop all of
the agents without failure checks being performed.

.. index:: ping.server.ip

Use the ``ping.server.ip`` property to specify the IP address of a server that
...
.. index:: ping.server.command

Use the ``ping.server.command`` property to specify the command used to test
...
.. _stable.nodes.file:
.. index:: stable.nodes.file

Use the ``stable.nodes.file`` property to instruct the server to not rewrite
...
.. index:: db.reuse.connection.count
...
.. index:: auto.reconfigure

Use the ``auto.reconfigure`` property to instruct Failover Manager to enable
or disable automatic reconfiguration of remaining Standby servers after
the primary standby is promoted to Master. Set the property to ``true`` to
enable automatic reconfiguration (the default) or ``false`` to disable
...
the ``recovery.conf`` file will be backed up during the reconfiguration process.
...
Please note: ``primary_conninfo`` is a space-delimited list of keyword=value
pairs.
...
:doc:`Using the efm Utility <using_efm_utility>`.
...
.. _use.replay.tiebreaker:
.. index:: use.replay.tiebreaker
...
selects a replacement master.  Set the ``use.replay.tiebreaker`` property to ``true``
...
and promote a node based on user preference, set ``use.replay.tiebreaker`` to ``false``.
...
**Please note:** You should set the ``application.name`` property on the master and
...
See the PostgreSQL documentation for more information about using a `restore_command <https://www.postgresql.org/docs/12/runtime-config-wal.html#RUNTIME-CONFIG-WAL-ARCHIVE-RECOVERY>`_.
...
.. _reconfigure.num.sync:
.. index:: reconfigure.num.sync
...
The synchronous_standby_names parameter on the master node specifies the names and count of the synchronous  standby servers that will confirm receipt of data, to ensure that the master nodes can accept write transactions.  When set to true, Failover Manager will reduce the number of synchronous standby servers and reload the configuration of the master node to reflect the current value.
 ...
 # value required by the master database. If set to true, Failover Manager will
 # reduce the number of standbys needed in the master's
 # synchronous_standby_names property and reload the master configuration.
 # Failover Manager will not reduce the number below 1, taking the master
 # out of synchronous replication, unless the reconfigure.sync.master
 # property is also set to true.
 reconfigure.num.sync=false

.. _reconfigure.sync.master:
.. index:: reconfigure.sync.master

Set the ``reconfigure.sync.master`` property to ``true`` to take the master database
out of synchronous replication mode if the number of standby nodes drops below the
level required.  Set ``reconfigure.sync.master`` to ``false`` to send a notification
...
.. index:: recovery.check.period

Use the ``recovery.check.period`` property to specify the number of seconds
...
.. _restart.connection.timeout:
.. index:: restart.connection.timeout
...
For information about using a virtual IP address, see
:doc:`Using Failover Manager with Virtual IP Addresses <using_vip_addresses>`.
...
.. _check.vip.before.promotion:
.. index:: check.vip.before.promotion
...
.. _script.load.balancer:
.. index:: script.load.balancer.attach
...
Provide a script name after the ``script.load.balancer.attach`` property to
identify a script that will be invoked when a node should be attached to
the load balancer. Use the ``script.load.balancer.detach`` property to
specify the name of a script that will be invoked when a node should be
detached from the load balancer. Include the ``%h`` placeholder to represent
the IP address of the node that is being attached or removed from the
cluster. Include the ``%t`` placeholder to instruct Failover Manager to
...
.. index:: script.master.isolated

Use the ``script.master.isolated`` property to specify the complete path to
...
.. index:: script.remote.pre.promotion
...
.. index:: script.remote.post.promotion
...
.. index:: script.custom.monitor
           custom.monitor.interval
           custom.monitor.timeout
           custom.monitor.safe.mode

Use the ``script.custom.monitor`` property to provide the name and location
...
Use the ``sudo.command`` property to specify a command that will be invoked
...
specify the protocol type (UDP or TCP) and the ``syslog.port`` parameter to
specify the listener port of the syslog host. The ``syslog.facility`` value
...
`Enabling syslog Log File Entries <enabling_syslog>`_ .


.. _loglevel:
.. index:: jgroups.loglevel
           efm.loglevel
...
For more information about logging, see :doc:`Controlling Logging <controlling_logging>` .
