...
metaTitle: Connecting a Client Application to a CDS Cluster
metaDescription: description
---

To connect to a CDS hosted cluster, provide your client application with
the IP address and port of the server, and the credentials associated
with the role defined when the server cluster was created.
...
master server (the first Address name listed in the `Details` panel).
This will ensure that read requests are distributed efficiently across
the cluster replicas to maximize performance, while write requests are
directed only to the cluster master.

  - The `ADDRESS` column displays the address of the node; a connecting
  ...
  - The `LBPORT` column displays the port number to which a client
    application should connect to utilize load balancing. Since only the
    master node of a multi-server cluster operates in read/write mode,
    all write queries will be directed to the master node, while any
    read-only queries may be directed to a replica node.
...
To use the psql client to connect to an instance, navigate into the
directory in which the psql client is installed on your local system and
enter:

> `./psql -h <instance_address> -p 9999 -U <superuser_name> -d
> <database_name>`
...
> 
> *database_name* is the name of the database to which you will
> connect.
...
If connecting with a JDBC client, you must configure the client
connection to use SSL. For example, specify the following properties
when configuring the client JDBC driver:
...
For more information about configuring a JDBC client, please see the
[JDBC Connector
Guide](https://www.enterprisedb.com/docs/en/11.0.2/JDBC_Guide_11.0.2/EDB_Postgres_Advanced_Server_JDBC_Connector_Guide.1.01.html)

## Load Balancing

CDS uses pgPool functionality to implement automatic load balancing.
...
CDS monitors the health of the load balancer to ensure that service is
not interrupted. If the load balancer (pgpool) should fail while
monitoring is enabled, PgPool will be automatically restarted. If the
load balancer cannot be automatically restarted, CDS will display a
warning sign next to the cluster name on the Details panel and send a
notification email to the cluster user.

To utilize load balancing, you should direct client applications to
connect to the load balancing port (port 9999). The load balancing port
number is displayed in the LBPORT column on the Details pane of the
Clusters tab of the console.
