...
> `<server_name>` is the name of the database server whose backups are to be deleted.
  ...
  - `-i` (or ``--backupid) { all | [']{ <backup_id> | <backup_name> }',... }[`] }``
      ...
      - Multiple backup identifiers and backup names may be specified in a comma-separated list. The list must be enclosed within single quotes if there is any white space appearing before or after each comma (see [Example](../01_using_subcommands/03_delete#deleting_multiple_backups_with_space_characters) section).
