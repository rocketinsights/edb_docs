...
**Step 2:** Inspect the `pg_xlog` subdirectory of the data directory and save any WAL files that have not yet been archived to the `archive_path`. If there are files that have not been archived, save them to a temporary location.
...
>   - If you do not specify `-t timeline_id`, `-x target_xid`, or `-g target_timestamp`, then minimal recovery settings will be saved in the `postgresql.auto.conf` file and archive recovery will proceed only until consistency is reached, with no restoration of files from the archive. See the
