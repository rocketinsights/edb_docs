...
The `DELETE` subcommand removes the subdirectory and data files from the BART backup catalog for the specified backups along with its archived WAL files. To view examples of `DELETE` subcommand, see the *EDB Postgres Backup and Recovery Reference Guide* available at:
...
For database servers under a retention policy, there are conditions where certain backups may not be deleted. See the [Section - Deletions Permitted Under a Retention Policy](deletions_permitted_under_retention_policy) for
