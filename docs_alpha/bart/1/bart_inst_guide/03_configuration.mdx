...
<!-- end list -->
  ...
  - \- `User` (required) – Specify the replication database user name used  
    by BART to:
    ...
    While running as the BART user, the connection to the database server must not prompt for a password. The `pg_hba.conf` file must contain a replication connection entry for this database user name. See

[Setting up a Replication Database User](setting_up_a_replication_database_user) for more information.
...
[Archive Command Auto Configuration](archive_command_auto_configuration) for additional information.
...
[Configuring the BART host](configuring_the_bart_host) section for information about configuring the following optional parameters.
...
<!-- end list -->
  ...
  - \- Set `max_wal_senders` to a value high enough to leave at least one  
    session available for the backup. If the `xlog_method=stream` parameter setting is to be used by this database server as determined in the BART configuration file, the `max_wal_senders` setting must account for an additional session for the transaction log streaming (the setting must be a minimum of 2). See

[Configuring the BART host](configuring_the_bart_host) for information about the `xlog_method` parameter.
