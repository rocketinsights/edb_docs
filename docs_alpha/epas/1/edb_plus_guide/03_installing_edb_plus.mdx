...
metaTitle: Installing EDB*Plus
metaDescription: description
---

You can use an RPM installer or a graphical installer to add EDB*Plus
to your Advanced Server installation.
...
Installing EDB*Plus with an RPM
...
After saving your changes to the configuration file, you can use the
`yum install` command to install EDB*Plus:
...
You must confirm that an `identd` server is installed and running. You
can use the yum package manager to install an `identd` server by
invoking the command:

> `yum install xinetd authd`

The command should create a file named `/etc/xinetd.d/auth` that
contains:
...
if the file includes a `-E` argument at the end of the server arguments,
please erase `-E`.
...
``` text
---
title: map_name    system_username     postgres_username
metaTitle: map_name    system_username     postgres_username
metaDescription: description
---
...
Where:
  ...
  - The name specified in the `map_name` column is a user-defined name
    that will identify the mapping in the `pg_hba.conf` file.
  - The name specified in the `system_username` column is
    `enterprisedb`.
  - The name specified in the `postgres_username` column is
    `enterprisedb`.

Then, open the `pg_hba.conf` file and modify the IDENT entries:

  - If you are using an IPv4 local connection, modify the file entry to
    read:
    
    > `host all all 127.0.0.0/0 ident map=edbas`

  - If you are using an IPv6 local connection, modify the file entry to
    read:
...
Installing EDB*Plus through graphical installer
...
you wish to install the EDB*Plus software. Then, click `Next` to
continue.
...
window to identify the location of the Advanced Server host:
...
the information needed to install EDB*Plus on your system. Click `Next`
to install EDB*Plus.
...
EDB*Plus installation. Click `Finish` to exit the installer.
