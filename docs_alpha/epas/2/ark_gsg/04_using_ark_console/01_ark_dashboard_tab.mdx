...
To launch a cluster from the `Dashboard` tab, use the `Tenant` drop-down listbox to select the tenant in which the cluster will be created. Then, use the `Launch DB Cluster` button located in the `Getting Started` panel to open the `Create New Cluster` dialog and define the cluster attributes. For more information about defining a cluster, see `Creating a Server Cluster <creating_a_cluster>` .
