...
![*Compilation of a program containing embedded SQL
statements*](images/ecpg_path.png)
...
`my_program.pgc` into C code that calls the `ecpglib` API, and produces
a C program (`my_program.c`). Then, pass the C program to a C compiler;
the C compiler generates an object file (`my_program.o`). Finally, pass
the object file (`my_program.o`), as well as the `ecpglib` library file,
and any other required libraries to the linker, which in turn produces
the executable (`my_program`).
...
In the above code fragment:
  ...
  - Lines 4 through 6 contain C declarations.
    ...
    that each `VARCHAR` declaration is translated into an equivalent
    `struct` declaration.
...
Any SQL statement must be prefixed with `EXEC SQL` and extends to the
next (unquoted) semicolon. For example:
...
without translation and converts each `EXEC SQL` statement into a call
to an `ecpglib` function. The result would appear similar to the
following:
...
wizard as part of the `Database Server` component. On Linux, install
with the `edb-as<xx>-server-devel` RPM package where *xx* is the
Advanced Server version number. By default, the executable is located
in:
...
> `set EDB_PATH=C:\Program Files\edb\as12\bin` `set
> PATH=%EDB_PATH%;%PATH%`
...
A `makefile` contains a set of instructions that tell the `make` utility
how to transform a program written in C (that contains embedded SQL)
into a C program. To try the examples in this guide, you will need:
...
The following code is an example of a `makefile` for the samples
included in this guide. To use the sample code, save it in a file named
`makefile` in the directory that contains the source code file.
...
compiler and `LDFLAGS` and `LDLIBS` when invoking the linker. ECPG
programs must be linked against the ECPG run-time library (`-lecpg`) and
the libpq library (`-lpq`)
...
The sample `makefile` instructs make how to translate a `.pgc` or a
`.pc` file into a C program. Two lines in the `makefile` specify the
mode in which the source file will be compiled. The first compile option
is:
...
The first option tells `make` how to transform a file that ends in
`.pgc` (presumably, an ECPG source file) into a file that ends in `.c`
(a C program), using community ECPG (without the ECPGPlus enhancements).
It invokes the ECPG pre-compiler with the `-c` flag (instructing the
compiler to convert SQL code into C), using the value of the `INCLUDES`
variable and the name of the `.pgc` file.
...
Pro*C-compatibility mode), using the value of the `INCLUDES` variable
...
compile `customer_list.pgc` into a C program (`customer_list.c`), and
then uses the rules built into `make` to compile `customer_list.c` into
...
In the sample `makefile` shown above, `make` includes the `-C` option
when invoking ECPGPlus to specify that ECPGPlus should be invoked in
Pro*C compatible mode.

If you include the `-C` `PROC` keywords on the command line, in addition
to the ECPG syntax, you may use Pro*C command line syntax; for example:
...
<tr class="even">
<td><p>-C <em>mode</em></p></td>
<td><p>Use the <em>-C</em> option to specify a compatibility mode: <code>INFORMIX</code></p>
<p><code>INFORMIX_SE</code></p>
<p><code>PROC</code></p></td>
</tr>
<tr class="odd">
<td><p>-D <em>symbol</em></p></td>
<td><p>Define a preprocessor <em>symbol</em>.</p>
<p>The <em>-D</em> keyword is not supported when compiling in <em>PROC mode.</em> Instead, use the Oracle-style <em>‘DEFINE=’</em> clause.</p></td>
</tr>
<tr class="even">
<td>-h</td>
<td>Parse a header file, this option includes option <em>c</em>.</td>
</tr>
<tr class="odd">
<td>-i</td>
...
<tr class="even">
<td><p>-r <em>option</em></p></td>
<td><p>Specify run-time behavior; <em>option</em> can be:</p>
<p><code>no_indicator</code> - Do not use indicators, but instead use special values to represent NULL values.</p>
<p><code>prepare</code> - Prepare all statements before using them.</p>
<p><code>questionmarks</code> - Allow use of a question mark as a placeholder.</p>
<p><code>usebulk</code> - Enable bulk processing for INSERT, UPDATE and DELETE statements that operate on host variable arrays.</p></td>
</tr>
<tr class="odd">
<td>--regression</td>
...
<tr class="even">
<td>-t</td>
<td>Turn on <code>autocommit</code> of transactions.</td>
</tr>
<tr class="odd">
<td>-l</td>
<td>Disable <code>#line</code> directives.</td>
</tr>
<tr class="even">
<td>--help</td>
...
If you do not specify an output file name when invoking ECPGPlus, the
output file name is created by stripping off the `.pgc` filename
extension, and appending `.c` to the file name.
