...
![*The Stack Builder welcome
window*](images/the_stack_builder_welcome_window.png)
...
use the `Proxy Servers` button on the `Welcome` window to open the
...
proxy` or `FTP proxy` fields on the `Proxy servers` dialog. Currently,
all Stack Builder modules are distributed via HTTP proxy (FTP proxy
information is ignored). Click `OK` to continue.

![*The Stack Builder module selection
window*](images/the_Stack_Builder_module_selection_window.png)

The tree control on the Stack Builder module selection window contains a
node for each module category; click on a category heading to expose the
modules within that category.

Each entry within the tree control is the name of a module that can be
installed with Stack Builder.

  - If the module is installed, you will see the word (`installed`) to
  ...
  - To add new modules to the selected PostgreSQL installation, check
    the box to the left of the module name and click `Next`.

![*Check the box to add new
module*](images/check_the_box_to_add_new_module.png)

The `Selected packages` window confirms the packages selected.

![*A summary window displays a list of selected
packages*](images/a_summary_window_displays_a_list_of_selected_packages.png)
...
complete*](images/download_process_is_complete.png)

You can check the box next to `Skip Installation` and select `Next` to
exit Stack Builder without installing the downloaded files, or leave the
...
The modules supported by Stack Builder are subject to change and vary by
platform.
