...
metaTitle: Using the Procedural Languages
...
before creating a PL/Perl function. Use the `` `CREATE LANGUAGE ``
command at the EDB-PSQL command line to install PL/Perl. Open the
EDB-PSQL client, establish a connection to the database in which you
wish to install PL/Perl, and enter the command:
...
PL/Python example creates a function named `pymax` that returns the
larger of two integer values:
