...
metaTitle: Tutorial - Configuring a Simple Failover Manager Cluster
...
production system, please see `The Cluster Properties File
<cluster_properties>` for a complete list of properties.
...
EFM uses the `db.service.name` and `db.bin` properties when restarting
the server. The service name provided with the `db.service.name`
property is used when restarting the server with `service` or
`systemctl`; the value you provide in the `db.bin` property (the path to
the Postgres `bin` directory), will be used for calls to `pg_ctl`.
Please note that `db.bin` is a required field. `db.service.name` is
...
The data directory in which EFM will find or create the `recovery.conf`
file or the `standby.signal` file:
...
Set this property to `true` on a witness node and `false` if it is a
master or standby:
...
can be used to simplify authorization of subsequent nodes.
...
yet). For more information about the `efm.nodes` file, see `The Cluster
Members File <cluster_members>`
...
`edb-efm-3.9`; you can use your platform-specific service command to
control the service. For example, on a CentOS or RHEL 7.x host use the
command:
...
For detailed information about using the efm command line tool, see
