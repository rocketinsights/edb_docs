...
metaTitle: Performance Diagnostics
...
For more information about EDB Wait States, see the *EDB Postgres
...
**Prerequisites**
  ...
  - You must ensure that the EDB Wait States module is installed. Modify
    the `postgresql.conf` file, adding the `edb_wait_states` library to
    the list of libraries in the `shared_preload_libraries` parameter:
    ...
    The console will display an error if you do not meet the
    prerequisites.

![*The prerequisites
error*](images/performance_diagnostic_prereq_error.png)
...
By default, the top most Performance Diagnostic chart pulls the data for
one hour, starting from current date and time. The default range
selection in hours in the first chart can be customized in the
`Performance Diagnostic` section of the `Preferences` dialog under the
...
Tab?` to display the `Performance Diagnostics` dashboard in a new
browser tab.

![*The Performance Diagnostics
chart*](images/performance_diagnostic_chart.png)
...
period. The second chart displays the total number of wait events of
each type, for the timeline that you select in the first chart. You can
select the specific wait event types for which you want to analyze the
data.

To make differentiation easier, the graph for each wait event type is
displayed in a different color. Click on `Legend` to identify the color
in which a particular wait event type is displayed in the graph.

![*Selecting a
timeline*](images/performance_diagnostic_select_timeline.png)

Select a point in time on the second chart for which you wish to analyze
the wait events; the `Wait Event Details` panel is populated on the
basis of your selection in the second chart. The panel makes wait
details available on three tabs:

  - The `SQL tab` displays the list of SQL queries having wait events
    for the selected sample time.
...
![*Reviewing query
details*](images/performance_diagnostic_review_query_details.png)
...
The `Waits information` table displays the waiting query. If the query
is partially displayed, click the down arrow at the bottom of the
section to view the complete SQL query.

![*The Waits information
panel*](images/performance_diagnostic_wait_information_panel.png)

The `Wait Types` panel displays the total number of wait events for the
selected session ID and query ID. Select a time range in the first chart
to analyze the data for a specific period.
...
![*The wait types bar
graph*](images/performance_diagnostic_wait_type_bargraph.png)
...
![*The wait events donut
chart*](images/performance_diagnostic_wait_event_donut_chart.png)

In the `Wait Events` donut chart, all the wait event types applicable to
the selected timeline are displayed a percentage format. You can select
any one wait event type to see all the wait events belonging to that
particular wait event type and their count. Click the `Read More` link
to read about the various wait event types and wait events.
