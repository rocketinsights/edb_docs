...
Console controls make it simple to add replicas and storage to an
...
The `Scale Up` dialog makes it simple to manually add additional
...
If you specify that CDS should add both storage and replicas, CDS will
process the request for additional storage *before* adding replicas to
the cluster. All of the nodes on the cluster will be of the newly
specified storage size. To add a replica or storage space to a cluster,
navigate to the `Clusters` tab, highlight a cluster name, and select the
`Scale Up` icon. The `Scale Up` dialog opens as shown below.
...
When you've completed the dialog, click `Next` to continue to the
`Step 2` tab.

![*The Scale Up dialog.*](images/cds_manual_scaling_st2.png)

Use the `Previous` button to return to the `Step 1` tab to modify
specified values. Use the `Scale Up` button to confirm that you wish to
add the specified number of replication servers or the specified amount
of memory to the cluster. Use the `Cancel` button, or simply close the
dialog to exit without modifying the cluster.
...
CDS's `Scale Down` dialog makes it simple to manually remove one or more
unneeded replicas from a cluster. To delete a replica, navigate to the
`Clusters` tab, and click the `Scale Down` icon. The Scale Down dialog
...
![*The Step 2 tab of the Scale Down
dialog.*](images/cds_scaling_down_st2.png)

Click `Scale Down` to confirm that you wish to remove the replica, or
`Previous` to return to the `Step 1` tab. Use the `Cancel` button, or
...
vertically scale to a larger server class. Use the `Scale Machine Type`
dialog to copy the cluster into a larger server class.

When you use the `Scale Machine Type` dialog to move your cluster into a
larger server class, you must provide a new name for the upgraded
cluster. You can also use the dialog to specify that CDS should
...
dialog, CDS will copy the existing cluster into a new cluster of a
different server class, and optionally re-assign the IP address of the
existing cluster to the new cluster. You can scale your cluster from any
region.

To open the `Scale Machine Type` dialog, navigate to the `Clusters` tab,
and select the `Scale Machine Type` icon.
...
When you click the `Scale` button to start scaling the cluster, CDS will
confirm that the scaling is in progress.

![*The cluster is being scaled.*](images/cds_new.png)

Before creating the new cluster and (optionally) re-assigning the IP
address, CDS will perform a backup of the original cluster. During the
process, status indicators in the `PENDING` column of the `Clusters` tab
will keep you informed as CDS backs up the original cluster, and
initializes the new cluster.
