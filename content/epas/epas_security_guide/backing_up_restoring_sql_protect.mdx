...
metaTitle: Backing Up and Restoring a SQL/Protect Database
...
database with new OIDs such as is the case when using the pg\_dump
backup program.
...
SQL/Protect uses two tables (`edb_sql_protect` and
`edb_sql_protect_rel`) to store information on database objects such as
databases, roles, and relations. References to these database objects in
these tables are done using the objects’ OIDs, and not the objects’ text
names. The OID is a numeric data type used by Advanced Server to
uniquely identify each database object.
...
relations stored in the `edb_sql_protect` and `edb_sql_protect_rel`
tables are no longer valid when these tables are simply dumped to a
backup file and then restored to a new database.
...
The following steps back up a database that has been configured with
SQL/Protect.

**Step 1:** Create a backup file using pg\_dump.

The following example shows a plain-text backup file named
`/tmp/edb.dmp` created from database `edb` using the `pg_dump` utility
program:
...
function (where *sqlprotect\_file* is the fully qualified path to a file
...
The files `/tmp/edb.dmp` and `/tmp/sqlprotect.dmp` comprise your total
database backup.
...
**Step 5:** Delete any offending queries that may exist for the
database.
...
where *sqlprotect\_file* is the fully qualified path to the file:
...
Tables `edb_sql_protect` and `edb_sql_protect_rel` are now populated
with entries containing the OIDs of the database objects as assigned in
the new database. The statistics view `edb_sql_protect_stats` also now
...
`edb_sql_protect_rel`:
  ...
  - **dbid.** Matches the value in the `oid` column from `pg_database`
    for `newdb`
  - **roleid.** Matches the value in the `oid` column from `pg_roles`
    for `appuser`

Also note that in table `edb_sql_protect_rel`, the values in the `relid`
column match the values in the `oid` column of `pg_class` for relations
`dept` and `appuser_tab`.
