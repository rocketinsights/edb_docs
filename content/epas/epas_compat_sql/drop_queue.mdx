...
`DROP QUEUE` allows a superuser or a user with the
`aq_administrator_role` privilege to drop an existing queue.

**Parameters**

`name`

> The name (possibly schema-qualified) of the queue that is being
> dropped.
