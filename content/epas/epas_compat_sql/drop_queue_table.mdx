...
> error if the queue table does not exist. The server will issue a
> notice.
...
> Include the `RESTRICT` keyword to instruct the server to refuse to
> delete the queue table if any objects depend on it. This is the
> default.
...
<span class="title-ref">CREATE QUEUE TABLE
\<create_queue_table\>_</span>,
