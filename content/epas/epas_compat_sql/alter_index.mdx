...
Use the second variation of the `ALTER INDEX` command to rebuild an
index:
...
clause changes the name of the index. The `REBUILD` clause reconstructs
an index, replacing the old copy of the index with an updated version
based on the index's table.
