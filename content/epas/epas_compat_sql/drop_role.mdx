...
`DROP ROLE` removes the specified role. To drop a superuser role, you
must be a superuser yourself; to drop non-superuser roles, you must have
...
> as the role (and all objects owned by the role belonging to the
> schema) as long as no other dependencies on the role or the schema
> exist.
...
`CREATE ROLE <create_role>`, `SET ROLE <set_role>`, `GRANT <grant>`,
`REVOKE <revoke>`
