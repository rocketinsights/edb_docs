...
occurs, the OCI reports `ORA-08177: can't serialize access for this
transaction`, and the application must retry the transaction.
...
> install
> <span class="title-ref">postgres_fdw</span><span class="title-ref">.
> For more information, please see the
> </span><span class="title-ref">CREATE EXTENSION</span>\` command in
...
`LD_LIBRARY_PATH` so it is in effect when the script invokes the `pg_ctl
utility`.
...
For more information about foreign servers, please see the `CREATE
SERVER` command in the PostgreSQL Core documentation at:
...
<span class="title-ref">DROP PUBLIC DATABASE LINK
\<drop_public_database_link\>_</span>
