...
> `CREATE DIRECTORY` command does not create the operating system
> directory. The physical directory must be created independently using
> the appropriate operating system commands.

**Notes**

The operating system user id, `enterprisedb`, must have the appropriate
read and/or write privileges on the directory if the `UTL_FILE` package
is to be used to create and/or read files using the directory.
...
`SYS.ALL_DIRECTORIES` and `SYS.DBA_DIRECTORIES`, which are compatible
...
Create an alias named `empdir` for directory `C:\TEMP\EMPDIR` on
Windows:
...
<span class="title-ref">DROP DIRECTORY \<drop_directory\>_</span>
