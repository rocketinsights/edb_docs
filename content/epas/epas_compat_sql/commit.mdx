...
Use `ROLLBACK` to abort a transaction. Issuing `COMMIT` when not inside
a transaction does no harm.
...
`ROLLBACK <rollback>`, `ROLLBACK TO SAVEPOINT <rollback_to_savepoint>`
