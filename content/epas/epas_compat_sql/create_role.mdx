...
`LOGIN` attribute, so it cannot be used to start a session. Use the
`ALTER ROLE` command to give the role `LOGIN` rights. You must have
`CREATEROLE` privilege or be a database superuser to use the `CREATE
ROLE` command.

If the `IDENTIFIED BY` clause is specified, the `CREATE ROLE` command
also creates a schema owned by, and with the same name as the newly
created role.
...
Use `GRANT` and `REVOKE` to add and remove members of roles that are
being used as groups.
...
`ALTER ROLE <alter_role_identified_by>`, `DROP ROLE <drop_role>`, `GRANT
<grant>`, `REVOKE <revoke>`, `SET ROLE <set_role>`
