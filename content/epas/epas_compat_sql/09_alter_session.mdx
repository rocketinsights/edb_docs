...
> The following configuration parameters can be modified using the
...
> setting the Advanced Server `datestyle` runtime configuration
> parameter.
...
> Valid values are `BYTE` and `CHAR`. The default is `BYTE`. This
> parameter is provided for syntax compatibility only and has no effect
> in the Advanced Server.

`OPTIMIZER_MODE (string)`

> Sets the default optimization mode for queries. Valid values are
> `ALL_ROWS`, `CHOOSE`, `FIRST_ROWS`, `FIRST_ROWS_10`, `FIRST_ROWS_100`,
> and `FIRST_ROWS_1000`. The default is `CHOOSE`. This parameter is
> implemented in Advanced Server.

`QUERY_REWRITE_ENABLED (string)`

> Valid values are `TRUE`, `FALSE`, and `FORCE`. The default is `FALSE`.
> This parameter is provided for syntax compatibility only and has no
> effect in Advanced Server.

`QUERY_REWRITE_INTEGRITY (string)`

> Valid values are `ENFORCED`, `TRUSTED`, and `STALE_TOLERATED`. The
> default is `ENFORCED`. This parameter is provided for syntax
> compatibility only and has no effect in Advanced Server.

**Examples**

Set the language to U.S. English in UTF-8 encoding. Note that in this
example, the value, `en_US.UTF-8`, is in the format that must be
specified for Advanced Server. This form is not compatible with Oracle
databases.
