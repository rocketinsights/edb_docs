...
> than one constraint matching the specified name. All matching
> constraints will be dropped.

`RENAME`

> The `RENAME` forms change the name of a table (or an index, sequence,
> or view) or the name of an individual column in a table. There is no
...
space. Adding a `CHECK` or `NOT NULL` constraint requires scanning the
...
<span class="title-ref">CREATE TABLE \<create_table\>_</span> for a
further description of valid
...
<span class="title-ref">CREATE TABLE \<create_table\>_</span>,
