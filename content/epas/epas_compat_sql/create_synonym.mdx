...
`CREATE SYNONYM` defines a synonym for certain types of database
objects. Advanced Server supports synonyms for:
...
> If you do not specify a schema name, the synonym is created in the
> first existing schema in your search path.
...
> `dblink_name` specifies the name of the database link through which an
> object is accessed.

Include the `REPLACE` clause to replace an existing synonym definition
with a new synonym definition.

Include the `PUBLIC` clause to create the synonym in the `public`
schema. The `CREATE PUBLIC SYNONYM` command, compatible with Oracle
databases, creates a synonym that resides in the `public` schema:
...
<span class="title-ref">DROP SYNONYM \<drop_synonym\>_</span>
