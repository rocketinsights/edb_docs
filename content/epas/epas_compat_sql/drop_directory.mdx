...
`DROP DIRECTORY` drops an existing alias for a file system directory
path that was created with the `CREATE DIRECTORY` command. To execute
this command you must be a superuser.
...
<span class="title-ref">CREATE DIRECTORY \<create_directory\>_</span>
