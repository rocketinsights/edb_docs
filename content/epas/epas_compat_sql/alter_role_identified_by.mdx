...
metaTitle: ALTER ROLE… IDENTIFIED BY
...
their own password. An unprivileged role must include the `REPLACE`
clause and their previous password if `PASSWORD_VERIFY_FUNCTION` is not
`NULL`<span class="title-ref"> in their profile. When the
</span><span class="title-ref">REPLACE</span>\` clause is used by a
non-superuser, the server will compare the password provided to the
existing password and raise an error if the passwords do not match.
