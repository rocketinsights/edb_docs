...
`UPDATE` command must not return more than one row, otherwise an
exception is thrown. If the result set is empty, then the contents of
the target record or variables are set to null.
...
`UPDATE` command may contain none, one, or more rows.
...
as the `SELECT` privilege to any table whose values are read in
...
> 
> > If the column specified in `return_expression` is assigned a value
> > in the `UPDATE` command, then the assigned value is used in the
> > evaluation of `return_expression`.
> > 
> > If the column specified in `return_expression` is not assigned a
> > value in the `UPDATE` command, then the column’s current value in
> > the affected row is used in the evaluation of `return_expression`.
