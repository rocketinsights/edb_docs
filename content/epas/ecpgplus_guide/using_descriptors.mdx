...
4.  Use a `PREPARE` statement to parse and syntax-check the SQL
    statement.
...
7.  Prompt the user (if required) for a value for each input parameter.
    ...
    descriptor.
8.  Use a `DECLARE CURSOR` statement to define a cursor for the
    statement.
...
12. Use a `CLOSE CURSOR` statement to close the cursor and free any
    cursor resources.
...
<th><strong>Attribute Description</strong></th>
</tr>
</thead>
<tbody>
<tr class="odd">
...
<tr class="odd">
<td><p>DATETIME_INTERVAL_CODE</p></td>
<td><p>integer</p></td>
<td><p>If TYPE is 9:</p>
<blockquote>
<p>1 - DATE</p>
...
print\_meta\_data() that will print the metadata found in a descriptor:
...
The application connects to the database (using the default
credentials):
...
Next, the application allocates an SQL descriptor to hold the metadata
for a statement:
...
The application uses a PREPARE statement to syntax check the string
...
and a DESCRIBE statement to move the metadata for the query into the SQL
descriptor.
...
If the statement executes successfully, the application performs a
COMMIT:
...
The application interrogates the row descriptor (row\_desc) to copy the
...
fit into the val host variable (so we print \<truncated\>); otherwise,
the null indicator is 0 (meaning NOT NULL) so we print the value. In
each case, we prefix the value (or \<null\> or \<truncated\>) with the
name of the column.
...
The print\_meta\_data() function extracts the metadata from a descriptor
...
The application prints the column headers (defined at the beginning of
this application):
...
code found in the types\[\] array), it sets type\_name to the name of
the corresponding type; otherwise, it sets type\_name to "unknown".
