...
Refer to <span class="title-ref">displayResultSet
\<executing_query_with_unknown_number_of_variables\>_</span>
function for an example of how to allocate
...
value expressions, please refer to <span class="title-ref">Declaring
Host Variables \<declaring_host_variables\>_</span> section.
...
required by the program. Note that each *actual_argument* corresponds
...
*user_name* is a host variable that contains the role that the client
...
is established. *database_name* is the name of the database to which
...
> `DEFAULT` as the target database, do not include a *connection_name*
> or *credentials*.
...
*descriptor_name* is the name of the descriptor. This value may take
...
*select_statement* is the text of the `SELECT` statement that defines
...
*statement_name* specifies the identifier associated with the
statement.
...
*using_list* is a list of table expressions, allowing columns from
...
*cursor_name* is the name of the cursor to use in the `WHERE CURRENT
...
*host_variable_list* that is returned by the `DELETE` command after
each row is deleted:
...
specified in the `host` variable, :max_sal:
...
*array_size* is an integer value or a host variable that contains an
...
> Include the `DESCRIPTOR` *SQLDA_descriptor* clause to provide an
...
> \[USING \[SQL\] DESCRIPTOR \<descriptor_name\>\] \[INTO \[SQL\]
...
*descriptor_name* specifies the name of a descriptor (as a
...
*array_size* is an integer value or a host variable that contains an
integer value that specifies the number of rows to be processed. If you
specify an array_size, the host_variable must be an array of that
...
into the descriptor, parse_desc. The call to `EXEC SQL GET DESCRIPTOR`
...
Include the `FOR` *exec_count* clause to specify the number of times
...
> DESCRIPTOR \<SQLDA_descriptor\> or \<host_variable\> \[ \[ INDICATOR
> \] \<indicator_variable\>, … \]

Where:

*array_*size\* is an integer value or a host variable that contains an
integer value specifying the number of rows to fetch. If you omit the
...
*SQLDA_descriptor*, the descriptor must be initialized with a
`DESCRIBE` statement.
...
single row, *host_variables* must be an array.
