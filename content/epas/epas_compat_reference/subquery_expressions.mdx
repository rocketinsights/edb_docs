...
metaTitle: Subquery Expressions
...
The argument of `EXISTS` is an arbitrary `SELECT` statement, or
subquery. The subquery is evaluated to determine whether it returns any
rows. If it returns at least one row, the result of `EXISTS` is `TRUE`;
...
This simple example is like an inner join on `deptno`, but it produces
at most one output row for each `dept` row, even though there are
multiple matching `emp` rows:
...
only unequal subquery rows are found (including the special case where
the subquery returns no rows). The result is `FALSE` if any equal row is
found.
...
Note that if the left-hand expression yields `NULL`, or if there are no
equal right-hand values and at least one right-hand row yields `NULL`,
the result of the `NOT IN` construct will be `NULL`, not `TRUE`. This is
...
yield a Boolean result. The result of `ANY` is `TRUE` if any true result
is obtained. The result is `FALSE` if no true result is found (including
...
exactly one column. The left-hand expression is evaluated and compared
to each row of the subquery result using the given operator, which must
yield a Boolean result. The result of `ALL` is `TRUE` if all rows yield
true (including the special case where the subquery returns no rows).
The result is `FALSE` if any false result is found. The result is `NULL`
if the comparison does not return `FALSE` for any row, and it returns
...
`NOT IN` is equivalent to `<> ALL`. As with `EXISTS`, it’s unwise to
assume that the subquery will be evaluated completely.
