...
present. There may not be any spaces or other characters embedded in the
...
`MULTISET` is an extension to `CAST` that converts subquery results into
a nested table type. The synopsis is:
