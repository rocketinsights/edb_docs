...
trailing spaces are disregarded when comparing two values of type
`CHAR`, and they will be removed when converting a `CHAR` value to one
...
`VARCHAR`, `VARCHAR2`, `NVARCHAR` and `NVARCHAR2` will store the shorter
...
semantically equivalent to `VARCHAR2` except no length limit is
specified. Generally, you should use a `CLOB` type if the maximum string
length is not known.
...
`pg_largeobject` system catalog. For information on large objects, see
