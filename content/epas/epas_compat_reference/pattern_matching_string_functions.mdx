...
metaTitle: Pattern Matching String Functions
metaDescription: description
---

Advanced Server offers support for the `REGEXP_COUNT`, `REGEXP_INSTR`
and `REGEXP_SUBSTR` functions. These functions search a string for a
pattern specified by a regular expression, and return information about
...
> source string at which `REGEXP_COUNT` will begin searching. The
> default value is `1`.
...
In the first example, the command instructs `REGEXP_COUNT` begins
counting in the first position; if we modify the command to start the
count on the 6th position:
...
`REGEXP_COUNT` returns `3`; the count now excludes any occurrences of
the letter `i` that occur before the 6th position.
...
> the string that `REGEXP_INSTR` should return. The default value is
> `0`. Specify:
> 
> > `0` to return the location within the string of the first character
> > that matches `pattern`.
> > 
> > A value greater than `0` to return the position of the first
> > character following the end of the `pattern`.
...
> `pattern` that will be returned by `REGEXP_INSTR`. The default value
...
> indicates which set of parentheses should be returned; for example, if
> `subexpression` is `2`, `REGEXP_INSTR` will return the position of the
...
digits begins in the 5<sup>th</sup> position.

## REGEXP_SUBSTR

The `REGEXP_SUBSTR` function searches a string for a pattern specified
by a POSIX compliant regular expression. `REGEXP_SUBSTR` returns the
string that matches the pattern specified in the call to the function.
...
> the value being searched for. The value specified by `subexpression`
> indicates which set of parentheses should be returned; for example, if
> `subexpression` is `2`, `REGEXP_SUBSTR` will return the value
