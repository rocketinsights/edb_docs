...
metaTitle: Pattern Matching Using the LIKE Operator
metaDescription: description
---

Advanced Server provides pattern matching using the traditional SQL
...
`LIKE` returns `TRUE`, and vice versa. An equivalent expression is `NOT
(string LIKE pattern)`.
...
(matches) any single character; a percent sign (`%`) matches any string
...
pattern anywhere within a string, the pattern must therefore start and
...
character with `ESCAPE`; then a backslash is not special to `LIKE`
anymore. (But it is still special to the string literal parser, so you
