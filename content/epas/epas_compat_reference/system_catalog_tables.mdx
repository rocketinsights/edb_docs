...
metaTitle: System Catalog Tables
...
| ----------------------------------------------------- | -------------------- | -------------- | ------------------------------------------------------------------------------------- |
| passhistroleid passhistpassword passhistpasswordsetat | oid text timestamptz | pg\_authid.oid | The ID of a role. Role password in md5 encrypted form. The time the password was set. |
...
The `edb_variable` table contains one row for each package level
variable (each variable declared within a package).
...
The `pg_synonym` table contains one row for each synonym created with
the `CREATE SYNONYM` command or `CREATE PUBLIC SYNONYM` command.
