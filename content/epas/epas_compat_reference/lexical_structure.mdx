...
instance the `UPDATE` command always requires a `SET` token to appear in
a certain position, and this particular variation of `INSERT` also
requires a `VALUES` token in order to be complete. The precise syntax
rules for each command are described in Database Compatibility for
Oracle Developers SQL Guide.
