...
The following example creates a new variable named my\_name, accepts a
...
directory of the Linux operating system account invoking EDB\*Plus (or
in the `%APPDATA%\postgresql\` directory for Windows) and then at the
...
the `pg_hba.conf` file. For more information about the `pg_hba.conf`
...
`localhost`, the port defaults to `5444` (which is not the same as the
port previously used), and the database defaults to `edb`.
...
`sal` and `comm`, respectively, are deleted from the SELECT command in
the SQL buffer.
...
The name of a specific topic – e.g., an EDB\*Plus command, for which
help is desired.
...
> `* n` displays a list of lines that range from the current line
> through line n.

`* L[AST]`

> `* L[AST]` displays a list of lines that range from the current line
...
`optional_text` specifies the text that will be displayed to the user.
If the `optional_text` is omitted, Advanced Server will display two
blank lines. If you double quote the `optional_text` string, the quotes
...
`message_text` specifies the text displayed to the user. Double quote
the string to include quotes in the output.
...
> *only* if a file with the specified name does not already exist. This
> is the default.
...
> to 0 is same as setting FEEDBACK to OFF. Setting `row_threshold` equal
> 1 effectively sets FEEDBACK to ON.
...
> Specify MIXED to leave the case of SQL commands unchanged. The default
> is MIXED.
...
Or display a specific parameter setting by including the parameter\_name
in the SHOW command:
...
Use the `output_file` parameter to specify a path name for the output
file.
...
Use the `variable_name` parameter to specify the name of a variable or
variables.
...
> 
> Include the `COMMIT` clause to instruct EDB\*Plus to COMMIT the
...
> 
> Include the `EXIT` clause to instruct EDB\*Plus to perform the
> specified action and exit if it encounters an error.
> 
