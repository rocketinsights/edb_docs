...
metaTitle: MIMEHEADER_ENCODE
metaDescription: description
---

Use the `MIMEHEADER_ENCODE` function to convert a string into mime
header format, and then encode the string. The signature is:
...
> string. You can specify:
  ...
  - `Q` to enable quoted-printable encoding. If you do not specify a
    value, `MIMEHEADER_ENCODE` will use quoted-printable encoding.
  - `B` to enable base-64 encoding.

**Examples**

The following examples use the `MIMEHEADER_ENCODE` and
`MIMEHEADER_DECODE` functions to first encode, and then decode a string:
