...
[Reply/Replies](reply/replies) for a description of `REPLY` and `REPLIES`.
...
> SMTP reply to the command. If SMTP returns multiple replies, only the last one is returned in `<reply>`. See
