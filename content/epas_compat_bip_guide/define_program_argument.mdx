...
metaTitle: DEFINE_PROGRAM_ARGUMENT
...
> argument. `<default_value>` will be overridden by a value specified by
> the job when the job executes.
...
The first argument is an `INTEGER` value named`dept_no` that has a
default value of `20`. The second argument is a `VARCHAR2` value named
`emp_name`; the second argument does not have a default value.
