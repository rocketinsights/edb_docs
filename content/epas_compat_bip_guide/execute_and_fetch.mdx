...
> result set is not exactly equal to 1. If set to `FALSE`, no exception
> is thrown. The default is `FALSE`. A `NO_DATA_FOUND` exception is
> thrown if `<exact>` is `TRUE` and there are no rows in the result set.
> A `TOO_MANY_ROWS` exception is thrown if `<exact>` is `TRUE` and there
