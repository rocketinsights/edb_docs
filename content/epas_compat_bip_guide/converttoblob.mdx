---
title: CONVERTTOBLOB
metaTitle: CONVERTTOBLOB
metaDescription: description
---

The `CONVERTTOBLOB` procedure provides the capability to convert
character data to binary.
...
> Position in bytes in the destination `BLOB` where writing of the
> source `CLOB` should begin. The first byte is offset 1.
...
> Position in characters in the source `CLOB` after the conversion
> operation completes. The first character is offset 1.
...
> Language context for the conversion. The default value of 0 is
> typically used for this setting.
...
> 0 if the conversion was successful, 1 if an inconvertible character
> was encountered.
