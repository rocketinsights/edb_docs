...
catalog tables in which the `DBMS_SCHEDULER` programs, schedules and
jobs are stored. Use the `psql` client to connect to the database, and
invoke the command:
...
using_calendar_syntax_to_specify_a_repeating_interval create_job
create_program create_schedule define_program_argument
dbms_scheduler_disable drop_job drop_program drop_program_argument
drop_schedule dbms_scheduler_enable evaluate_calendar_string
run_job set_job_argument_value
