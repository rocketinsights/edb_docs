...
The `DBMS_LOB` package provides the capability to operate on large
...
Advanced Server's implementation of `DBMS_LOB` is a partial
...
the large objects are `BLOBs`. Lengths and offsets are measured in
characters if the large objects are `CLOBs`.

<div class="toctree" data-maxdepth="5">

append compare converttoblob converttoclob copy erase
get_storage_limit getlength instr read substr trim write writeappend
