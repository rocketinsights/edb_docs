...
The following anonymous block creates a table named, `job`. Note that
DDL statements are executed immediately by the `PARSE` procedure and do
not require a separate `EXECUTE` step.
