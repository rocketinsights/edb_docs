...
> `DROP_QUEUE_TABLE` command when dropping a table that contain entries:
> 
> > If the target table contains entries and force is `FALSE`, the
> > command will fail, and the server will issue an error.
> > 
> > If the target table contains entries and force is `TRUE`, the
> > command will drop the table and any dependent objects.
