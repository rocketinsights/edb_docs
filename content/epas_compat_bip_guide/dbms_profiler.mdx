...
The `START_PROFILER` function/procedure starts a data collection
session. The function and procedure signatures are:
...
> An additional user-defined comment for the profiler session. The
> default value is ''.
...
`FLUSH_DATA`) function/procedure, `DBMS_PROFILER` writes those timers
and counters to a set of three tables:
...
specific privileges on the profiling tables (stored in the `SYS`
schema). This permits a non-privileged user to gather performance
statistics without exposing information that the administrator may want
to keep secret.
...
(The call to `start_profiler()` includes a comment that `DBMS_PROFILER`
...
7.  Terminate the profiling session and flush the performance
    information to the profiling tables:
...
10. Query the `plsql_profiler_rawdata` table to view a list of the wait
    event counters and wait event times:
...
11. Query the `plsql_profiler_data` view to review a subset of the
    information found in `plsql_profiler_rawdata` table:
