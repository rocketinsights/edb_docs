...
> `CLOB` large object locator to which the binary data is to be
> converted.
...
> Position in characters in the destination `CLOB` after the write
> operation completes. The first character is offset 1.

`<src_offset> IN`

> Position in bytes in the source `BLOB` where conversion to the
> destination `CLOB` should begin. The first byte is offset 1.
