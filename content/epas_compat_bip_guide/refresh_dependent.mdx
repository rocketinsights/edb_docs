...
> `<list>` is a `VARCHAR2` value that specifies the name of a
> materialized view, or a comma-separated list of materialized view
> names. The names may be schema-qualified.
...
> `<method>` is a `VARCHAR2` value that specifies the refresh method
> that will be applied to the specified view (or views). The only
> supported method is `C`; this performs a complete refresh of the view.
