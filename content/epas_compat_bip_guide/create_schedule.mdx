...
> `<start_date>` is a `TIMESTAMP WITH TIME ZONE` value that specifies
> the date and time that the schedule is eligible to execute. If a
...
> </div>
> 
> You must provide a value for either `<start_date>` or
> `<repeat_interval>`; if both `<start_date>` and `<repeat_interval>`
> are `NULL`, the server will return an error.
> 
> </div>
