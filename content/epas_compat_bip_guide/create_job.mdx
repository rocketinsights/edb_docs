...
> `<job_name>` specifies the optionally schema-qualified name of the job
> being created.
...
> the first time that the job is scheduled to execute. The default value
...
> `<program_name>` is the name of a program that will be executed by the
> job.
