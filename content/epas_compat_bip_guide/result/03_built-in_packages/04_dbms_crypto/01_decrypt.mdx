...
When invoked as a procedure, `DECRYPT` returns `BLOB` or `CLOB` data to
...
Note that when calling `DECRYPT`, you must pass the same cipher type,
key value and initialization vector that was used when `ENCRYPTING` the
target.
