...
The `UTL_URL` package will return the `BAD_URL` exception if the call to
a function includes an incorrectly-formed URL.

## ESCAPE

Use the `ESCAPE` function to escape illegal and reserved characters
within an URL. The signature is:
...
>   - If `<escape_reserved_chars>` is `TRUE`, `ESCAPE` will escape both
>     the illegal characters and the reserved characters in the
>     specified URL.
> 
...
> others; to review comprehensive rules about illegal characters, please
> refer to RFC 2396. Some *examples* of characters that are considered
...
If you include a value of TRUE for the `<escape_reserved_chars>`
...
The `ESCAPE` function escapes the reserved characters as well as the
...
> database encoding before being returned. If `<url_charset>` is `NULL`,
> the character will not be converted. The default value of
> `<url_charset>` is `ISO-8859-1`.
