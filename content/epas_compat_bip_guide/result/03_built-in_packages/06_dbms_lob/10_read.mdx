...
The `READ` procedure provides the capability to read a portion of a
large object into a buffer.
...
> be read, then `<amount>` returns 0 and a `DATA_NOT_FOUND` exception is
> thrown.
...
> `<buffer>` must be `RAW`. If `<lob_loc>` is a `CLOB`, then `<buffer>`
> must be `VARCHAR2`.
